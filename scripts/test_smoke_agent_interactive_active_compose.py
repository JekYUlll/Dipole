...
class InteractiveAgentActiveComposeSmokeTest(unittest.TestCase):
    def test_smoke_is_isolated_and_requires_explicit_active_profiles(self) -> None:
        ...
        self.assertIn('DIPOLE_AGENT_MEMORY_SMOKE:=0', script)
        self.assertIn('DIPOLE_AGENT_INTERACTIVE_MEMORY_TASK_QUEUE="dipole-agent-interactive-memory-', script)
        self.assertIn('agent-interactive-memory-smoke.yml', script)
        ...
        self.assertIn('compose logs --no-color --tail=120 core agent', script)
    ...
    def test_smoke_verifies_both_replay_outcomes_and_revokes_authority(self) -> None:
        ...
        self.assertIn('verify_waiting_locator_metric()', script)
        self.assertIn('dipole_agent_task_waiting_locator_total{outcome="online"}', script)
        self.assertIn('denied_task=$(start_task_and_wait_for_locator "deny-$(openssl rand -hex 6)" "/send This message must stay uncommitted.")\nverify_waiting_locator_metric', script)
        self.assertIn('resolve_twice "${denied_task}" "${denied_approval}" denied', script)
        self.assertIn('agent_message_baseline=$(mysql -e "SELECT COUNT(*) FROM messages WHERE sender_uuid =', script)
        self.assertIn("denied_expected=$'0\\t'\"${agent_message_baseline}\"", script)
        self.assertIn('[[ "${denied_effects}" == "${denied_expected}" ]]', script)
        self.assertIn('resolve_twice "${approved_task}" "${approved_approval}" approved', script)
        self.assertIn("approved_expected=$'1\\t1\\t'\"$((agent_message_baseline + 1))\"", script)
        self.assertIn('[[ "${approved_effects}" == "${approved_expected}" ]]', script)
