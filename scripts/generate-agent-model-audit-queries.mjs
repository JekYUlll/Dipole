...
const outputUrl = new URL("../agent-runtime/src/models/mysql-model-audit-queries.ts", import.meta.url);
const source = await readFile(sourceUrl, "utf8");
const entries = [...source.matchAll(/^-- name: (\w+) :\w+\n([\s\S]*?)(?=^-- name:|(?![\s\S]))/gm)];

if (entries.length !== 10) {
  throw new Error(`expected 10 Agent Model Audit queries, found ${entries.length}`);
}
