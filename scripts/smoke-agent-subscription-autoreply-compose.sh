...
# Exercises the opt-in subscription auto-reply path in an isolated project. A
# subscription-triggered active read completes, then the runtime autonomously
# replies to the owner's direct Agent conversation. There is no owner Signal:
# Core mints the write grant via AuthorizeSubscriptionMessage after the reply
# content is fixed. The deterministic local model keeps smoke data on-host.

root_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
project_name="${COMPOSE_PROJECT_NAME:-dipole-agent-subscription-autoreply-${RANDOM}-$$}"
scratch_dir=$(mktemp -d "${TMPDIR:-/tmp}/dipole-agent-subscription-autoreply.XXXXXX")
owner_telephone="13900000005"
agent_uuid="UAI000000000000000001"
grant_uuid="PROMOTION-SUBSCRIPTION-AUTOREPLY-${RANDOM}-$$"
...
: "${DIPOLE_AGENT_CANDIDATE_VERSION:=agent-runtime@subscription-autoreply-compose-smoke}"
: "${DIPOLE_AGENT_ACTIVE_KAFKA_GROUP_ID:=dipole-agent-active-autoreply-smoke-${RANDOM}-$$}"
: "${DIPOLE_AGENT_SUBSCRIPTION_ACTIVE_KAFKA_GROUP_ID:=dipole-agent-subscription-active-autoreply-smoke-${RANDOM}-$$}"
: "${DIPOLE_AGENT_SUBSCRIPTION_ACTIVE_TASK_QUEUE:=dipole-agent-subscription-autoreply-smoke-${RANDOM}-$$}"
...
compose_files=(
  ...
  -f "${root_dir}/deploy/microservices/agent-subscription-autoreply.yml"
...
cleanup() {
  ...
  if [[ "${KEEP_STACK:-0}" != "1" ]]; then
    ...
    printf 'Subscription auto-reply Compose stack retained: project=%s scratch=%s\n' "${project_name}" "${scratch_dir}" >&2
  fi
  exit "${status}"
}
...
# direct Agent conversation, and return the binding identities.
...
const register = await fetch("http://core:8081/api/v1/auth/register", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ nickname: "Auto Reply Owner", telephone, password: "smoke-pass-123" }) });
const registerBody = await register.json();
const ownerUuid = registerBody?.data?.user?.uuid;
if (register.status !== 200 || typeof ownerUuid !== "string") throw new Error(`register failed: ${register.status} ${JSON.stringify(registerBody)}`);
...
# Produce the triggering owner message; the subscription matcher creates one
# owner-scoped active read Task that must autonomously reply.
...
await new Promise((resolve, reject) => {
  ...
  socket.addEventListener("open", () => socket.send(JSON.stringify({ type: "chat.send", data: { target_uuid: agentUuid, content: "subscription autoreply smoke", client_message_id: `subscription-autoreply-${Date.now()}` } })));
...
# single completed Tool invocation and one Sync inbox fan-out per side.
reply_effects=$(mysql -e "SELECT
  ...
  (SELECT COUNT(*) FROM agent_approvals WHERE task_uuid = '${task_uuid}'),
  (SELECT COUNT(*) FROM agent_tool_invocations WHERE task_uuid = '${task_uuid}' AND status = 'completed')")
[[ "${reply_effects}" == $'1\t1\t1\t1\t1' ]] || { printf 'subscription auto-reply effects diverged (messages, distinct_client_ids, consumed_approvals, total_approvals, tool_invocations): %q\n' "${reply_effects}" >&2; exit 1; }
