...
cmd_backend() {
  ...
  echo "==> Backend built → dist/dipole-{server,gateway,message,search,migrate,cassandra-projector,search-indexer,search-backfill,search-reconcile,search-alias,cassandra-backfill,cassandra-reconcile}"
}
