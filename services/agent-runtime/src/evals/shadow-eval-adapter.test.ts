...
describe("Shadow evaluation adapter", () => {
  ...
  it("allows only the canonical no-discovery read skip without an authorization record", () => {
    const observed = {
      ...observation(),
      steps: [
        observation().steps[0]!,
    ...
    const manifest = parseShadowEvalManifest({
      ...
      labels: {
        outcome: { requiredOutputIds: ["task:completed"], forbiddenOutputIds: [] },
        trajectory: { steps: ["capability:conversation.read:skipped"], forbiddenSteps: [] },
        ...
        cost: { maximums: { modelCalls: 1, toolCalls: 2, totalTokens: 300, totalCostMicrousd: 1_000, latencyMs: 1_000 }, routePrices: [{ route: "gateway/primary", inputMicrousdPerMillionTokens: 2_000_000, outputMicrousdPerMillionTokens: 6_000_000 }] }
      }
    });

    const suite = buildShadowEvalSuite(manifest, observed);
    expect(suite.cases.find(item => item.category === "trajectory")?.observed).toEqual({ steps: ["context.compile", "capability:conversation.list:completed", "capability:conversation.read:skipped", "artifact:conversation_digest:v1"] });
    expect(suite.cases.find(item => item.category === "cost")?.observed).toMatchObject({ toolCalls: 2 });
  });
