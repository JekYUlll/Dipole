...
describe("MySQL Shadow evaluation observation store", () => {
  it("loads a bounded low-sensitivity snapshot in deterministic order", async () => {
    const execute = vi.fn()
      ...
      .mockResolvedValueOnce([[{ step_no: 1, capability_id: "conversation.list", status: "completed", attempt_count: 1, latency_ms: 2, authorization_resource_type: "conversation", authorization_resource_id: "*", authorization_action: "list", authorization_decision: "allowed" }]])
    ...
    await expect(store.load("TASK-1", "RUN-1")).resolves.toEqual({
      taskId: "TASK-1", taskStatus: "completed", runId: "RUN-1", runStatus: "completed", traceId: "trace:store-1",
      contextManifest: { selected: [], omitted: [] },
      steps: [{ stepNo: 1, capabilityId: "conversation.list", status: "completed", attemptCount: 1, latencyMs: 2, authorization: { resourceType: "conversation", resourceId: "*", action: "list", decision: "allowed" } }],
  ...
  it("fails closed when a Step contains a partial authorization record", async () => {
    const execute = vi.fn()
      .mockResolvedValueOnce([[{
        task_uuid: "TASK-1", task_status: "completed", run_uuid: "RUN-1", run_status: "completed", trace_id: "trace:store-1",
        context_manifest_json: JSON.stringify({ selected: [], omitted: [] })
      ...
      .mockResolvedValueOnce([[{
        step_no: 1, capability_id: "conversation.list", status: "completed", attempt_count: 1, latency_ms: 2,
        authorization_resource_type: "conversation", authorization_resource_id: null, authorization_action: "list", authorization_decision: "allowed"
      }]])
      .mockResolvedValueOnce([[]])
      .mockResolvedValueOnce([[]])
      .mockResolvedValueOnce([[]]);
