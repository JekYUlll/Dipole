...
The gate groups C++ attempts by Kafka topic, partition and offset. Deferred attempts may precede one final projected outcome. Eligibility requires the Go workload to have complete acceptance, persistence and receipt counts with zero settled lag, and every C++ coordinate to finish with all requested node batches observed, zero final rejection/backpressure, and clean Presence aggregates.
