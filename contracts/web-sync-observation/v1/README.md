...
`evidence.schema.json` 保存满窗后的 recording rule、告警原始响应、判定原因和三层 SHA-256：Session、最终快照以及完整 Evidence。Final Evidence 还必须绑定对象存储归档收据（opaque URI、object version、ETag 和未来的 retention 截止时间）；缺少收据时只能生成 `blocked`。`eligible` 仍只代表 A6 手册中的人工晋级门槛满足，不会修改 Web 构建模式或 Cassandra hydration 路由。
