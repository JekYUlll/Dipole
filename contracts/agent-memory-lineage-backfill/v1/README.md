...
After an independently controlled shared-environment deployment and rollback drill, provide `deployment-evidence.schema.json` to `agent-memory-lineage-deployment-evidence`. The CLI binds the deployment record to the rollout review receipt, runtime revision, configuration digest, migration 43, health check and rollback drill. It writes only a low-sensitive `recorded` receipt; it never connects to the environment or grants backfill authority. Missing or failed external evidence remains a hard stop.
