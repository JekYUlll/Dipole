-- Rollback is intentionally rejected when approved group action records exist.
-- Preserve their audit trail rather than deleting records to satisfy the old constraint.
ALTER TABLE agent_tool_invocations
    ...
    ADD CONSTRAINT chk_agent_tool_invocation_action_reference CHECK (
        ...
        OR (
            ...
            AND (
                (action_command_kind IN ('assistant_reply', 'group_reply') AND approval_uuid IS NULL)
                OR (action_command_kind = 'system_message' AND approval_uuid IS NOT NULL)
            )
        )
    );
