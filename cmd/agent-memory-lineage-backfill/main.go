...
func main() {
	...
	if err := run(*jobName, *ownerID, *operatorID, *batchSize, *leaseSeconds, *execute, *manifestPath, *manifestOut, *approvalPath, *receiptOut); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
}

func run(jobName, ownerID, operatorID string, batchSize, leaseSeconds int, execute bool, manifestPath, manifestOut, approvalPath, receiptOut string) error {
	...
	if execute {
		if strings.TrimSpace(operatorID) == "" || strings.TrimSpace(manifestPath) == "" || strings.TrimSpace(approvalPath) == "" || strings.TrimSpace(receiptOut) == "" {
			return fmt.Errorf("execute mode requires -operator, -manifest, -approval and -receipt-out")
		}
	...
	if approval.OperatorID != strings.TrimSpace(operatorID) {
		return fmt.Errorf("Memory lineage backfill approval operator mismatch")
	}
