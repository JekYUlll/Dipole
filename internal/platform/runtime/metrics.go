...
func StartMetrics(cfg config.Metrics, serviceName string, consumer *platformKafka.Consumer, collectors ...prometheus.Collector) (*platformObservability.MetricsServer, error) {
	if !cfg.Enabled {
		return nil, nil
	}
	if cfg.Address == "" {
...
func MarkReady(server *platformObservability.MetricsServer) {
	if server == nil {
		return
	}
	server.MarkReady()
	logger.Info("service runtime ready", zap.String("service", server.Service()), zap.String("health_address", server.Address()))
}

func CloseMetrics(server *platformObservability.MetricsServer) error {
	if server == nil {
		return nil
	}
	server.MarkNotReady()
