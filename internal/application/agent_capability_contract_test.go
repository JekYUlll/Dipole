...
func TestAgentCapabilityV1HasLanguageNeutralContract(t *testing.T) {
	...
	if !ok || len(operations) != 7 {
		t.Fatalf("Agent Capability contract must declare seven operations, got %#v", schema["x-dipole-operations"])
	}
	want := map[string]bool{
		"get_user_profile": true, "list_direct_messages": true, "list_conversations": true,
		"read_conversation": true, "search_conversations": true, "list_contacts": true, "send_system_message": true,
	}
	...
	if !ok || len(descriptors) != 7 {
		t.Fatalf("Agent Capability contract must publish seven descriptors, got %#v", schema["x-dipole-capabilities"])
	}
	...
	for _, id := range []string{
		...
		application.AgentCapabilityContactsList,
