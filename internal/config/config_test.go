...
func TestConfigDistKeepsDeliveryObservationShadowDisabled(t *testing.T) {
	...
	if v.GetBool("internal_rpc.agent_oauth_callback_handoff_enabled") {
		t.Fatal("Agent OAuth callback handoff transitions must remain opt-in")
	}
...
func TestConfigDistKeepsAgentTaskControlsDefaultOff(t *testing.T) {
	...
	if v.GetBool("gateway.agent_definition_enabled") {
		t.Fatal("Gateway Agent Definition control must remain default off")
	}
