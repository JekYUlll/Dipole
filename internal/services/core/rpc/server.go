...
func NewWithAgentArtifacts(cfg config.InternalRPC, capability application.CoreCapability, agentCapability application.AgentCapabilityV1, resolver application.AgentInvocationResolverV1, admission application.AgentRunAdmissionServiceV1, approvals application.AgentApprovalServiceV1, controls application.AgentTaskControlAuthorizerV1, projections application.AgentTaskWorkflowProjectionServiceV1, repairs application.AgentWorkflowRepairAuditServiceV1, subscriptions application.AgentEventSubscriptionResolverV1, subscriptionControls application.AgentEventSubscriptionControlServiceV1, definitionCatalog application.AgentDefinitionCatalogServiceV1, artifacts application.AgentArtifactServiceV1, artifactCatalog application.AgentArtifactCatalogServiceV1, toolAudits application.AgentToolInvocationAuditServiceV1, toolRounds application.AgentMCPToolRoundServiceV1, toolTerminals application.AgentMCPToolInvocationTerminalServiceV1, messageCommands application.AgentMessageCommandExecutionV1, approvalGrants application.AgentApprovalGrantResolverV1, promotionControls application.AgentRuntimePromotionControlServiceV1, promotionEvidence application.AgentRuntimePromotionEvidenceReviewServiceV1, readinessPublisher application.AgentMCPReadinessEvidencePublisherV1, readinessResolver application.AgentMCPReadinessEvidenceResolverV1, memoryControls application.AgentMemoryOwnerControlServiceV1, memoryCandidates application.AgentMemoryCandidateCatalogServiceV1, memoryPromotions application.AgentMemoryCandidatePromotionServiceV1, timeline application.AgentTaskTimelineStoreV1, memoryPromotionCommits application.AgentMemoryPromotionReceiptCommitServiceV1, repairExecutor application.AgentWorkflowRepairExecutorV1, memories ...application.AgentMemoryContextResolverV1) (*Server, error) {
	agentAdapter, err := agentgrpc.NewServerWithControlAndProjection(agentCapability, resolver, admission, approvals, controls, projections, repairs)
	if err != nil {
		return nil, fmt.Errorf("create Agent Capability rpc adapter: %w", err)
	}
	...
	if artifactCatalog != nil {
		if _, err := agentAdapter.WithArtifactCatalog(artifactCatalog); err != nil {
			return nil, fmt.Errorf("configure Agent Artifact catalog rpc adapter: %w", err)
		}
	}
	if timeline != nil {
	...
	if memoryCandidates != nil {
		if _, err := agentAdapter.WithMemoryCandidateCatalog(memoryCandidates); err != nil {
			return nil, fmt.Errorf("configure Agent Memory candidate catalog rpc adapter: %w", err)
		}
	}
