...
type AgentCommandMessages interface {
	...
	SendAssistantGroupMessageContext(ctx context.Context, assistantUUID, groupUUID, content, clientMessageID string) (*model.Message, []string, error)
...
func (c *LocalAgentCommandV1) SendMessage(ctx context.Context, command application.AgentMessageCommandV1) (*model.Message, error) {
	...
	case application.AgentMessageCommandGroupReplyV1:
		...
		message, _, err = c.messages.SendAssistantGroupMessageContext(ctx, agentUUID, groupUUID, content, clientMessageID)
	default:
		return nil, application.ErrAgentCommandDenied
	}
	if err == nil {
	...
	if receipt.Status != application.MessageCommandReceiptStatusCommitted || !agentCommandMessageMatchesV1(receipt.Message, command.Kind, agentUUID, principalUUID, conversationKey, content, clientMessageID) {
		return nil, application.ErrAgentCommandConflict
	}
	return receipt.Message, nil
}

func agentCommandMessageMatchesV1(message *model.Message, kind application.AgentMessageCommandKindV1, senderUUID, principalUUID, conversationKey, content, clientMessageID string) bool {
	targetUUID := principalUUID
	targetType := model.MessageTargetDirect
	wantType := model.MessageTypeAIText
	if kind == application.AgentMessageCommandSystemMessageV1 {
		wantType = model.MessageTypeSystem
	}
	if kind == application.AgentMessageCommandGroupReplyV1 {
		targetUUID = strings.TrimPrefix(conversationKey, "group:")
	...
	if message == nil || strings.TrimSpace(message.SenderUUID) != senderUUID || strings.TrimSpace(message.TargetUUID) != targetUUID ||
		message.TargetType != targetType || strings.TrimSpace(message.ConversationKey) != conversationKey ||
		strings.TrimSpace(message.ClientMessageID) != clientMessageID || strings.TrimSpace(message.Content) != content {
		return false
	}
	switch kind {
	case application.AgentMessageCommandAssistantReplyV1:
		return message.MessageType == wantType
	case application.AgentMessageCommandSystemMessageV1:
		return message.MessageType == wantType
	case application.AgentMessageCommandGroupReplyV1:
		return message.MessageType == wantType
