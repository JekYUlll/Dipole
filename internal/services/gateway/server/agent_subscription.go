...
func (c *AgentSubscriptionControlClient) CreateDefinition(ctx context.Context, principalUUID, profile string) (*AgentDefinitionCatalogItem, error) {
	...
	defer cancel()
	response, err := c.rpc.CreateAgentDefinition(callCtx, &agentv1.CreateAgentDefinitionRequest{
		Context: grpccommon.RequestContextFrom(ctx, principalUUID, "dipole-gateway"), TenantId: c.tenantID, Profile: profile,
	})
	if err != nil {
