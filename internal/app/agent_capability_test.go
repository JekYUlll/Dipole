...
func TestLocalAgentCapabilityV1RestrictsProfileSubjects(t *testing.T) {
	...
	if _, err := capability.GetUserProfile(context.Background(), "U100", "UAI", "U100"); err != nil || core.requested != "U100" {
		t.Fatalf("principal profile: requested=%q err=%v", core.requested, err)
	}
	if _, err := capability.GetUserProfile(context.Background(), "U100", "UAI", "UAI"); err != nil || core.requested != "UAI" {
		t.Fatalf("Agent profile: requested=%q err=%v", core.requested, err)
	}
	if _, err := capability.GetUserProfile(context.Background(), "U100", "UAI", "U999"); !errors.Is(err, application.ErrAgentCapabilityDenied) {
...
func TestLocalAgentCapabilityV1RoutesAuthorizedConversationReads(t *testing.T) {
	...
	denied, err := capability.ReadConversation(context.Background(), "U100", "U999", 20)
	...
	direct, err := capability.ReadConversation(context.Background(), "U100", "U200", 20)
	...
	group, err := capability.ReadConversation(context.Background(), "U100", "G1", 20)
...
func TestLocalAgentCapabilityV1ListsConversationsAndPreservesCommandContext(t *testing.T) {
	...
	items, err := capability.ListConversations(context.Background(), "U100", 10)
	...
	message, err := capability.SendSystemMessage(ctx, "UAI", "U100", "notice")
