...
// versions:
//   sqlc v1.31.1
// source: agent_memory_derived_lineage.sql
...
const getAgentMemoryDerivedImpact = `-- name: GetAgentMemoryDerivedImpact :one
SELECT
...
type GetAgentMemoryDerivedImpactParams struct {
	TenantID      string
	PrincipalUuid string
	MemoryUuid    string
}

type GetAgentMemoryDerivedImpactRow struct {
	MemoryRootUuid         string
	LineageVersions        int64
	DirectTaskReferences   int64
	UnindexedContextPlans  int64
	UnattributedModelTasks int64
	ModelCalls             int64
	ShadowPlans            int64
	ShadowSteps            int64
	Artifacts              int64
	ToolInvocations        int64
	MessageActions         int64
}

func (q *Queries) GetAgentMemoryDerivedImpact(ctx context.Context, arg GetAgentMemoryDerivedImpactParams) (GetAgentMemoryDerivedImpactRow, error) {
	row := q.db.QueryRowContext(ctx, getAgentMemoryDerivedImpact, arg.TenantID, arg.PrincipalUuid, arg.MemoryUuid)
	var i GetAgentMemoryDerivedImpactRow
	err := row.Scan(
		&i.MemoryRootUuid,
		&i.LineageVersions,
		&i.DirectTaskReferences,
		&i.UnindexedContextPlans,
		&i.UnattributedModelTasks,
		&i.ModelCalls,
		&i.ShadowPlans,
		&i.ShadowSteps,
		&i.Artifacts,
