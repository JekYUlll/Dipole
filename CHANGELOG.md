...
- 2026-09-03：完成 Subscription Auto-Reply 的隔离端到端闭环。认证 owner 可在创建 Definition 时显式选择 `subscription_autoreply` profile；该 profile 仅授予 `conversation.read` 与 `message.write`，默认 Definition 的只读行为保持不变。补齐 Core 两条兼容 gRPC 最小权限 allowlist 中遗漏的 `AuthorizeSubscriptionMessage`，使 `dipole-agent` 只能为订阅 scoped 的 owner-Agent 会话申请自主写 grant。Runtime 在 Kafka 解码后、订阅查询前丢弃当前 Agent 自身发送的消息，阻断自动回复回流为新任务。Remote GPU 在隔离、loopback-only Compose project 中验证：一次 owner 事件恰好产生一个 completed durable Task、一次 model run、一次 consumed `message.system.send` approval、一次 completed Tool Invocation、一条 owner-Agent 回复和两条 Sync Inbox 投影；项目、卷与临时 grant 均已清理。该 smoke 使用容器内确定性模型 stub，不能外推真实模型效果、共享 tenant、容量、语义预筛或默认路径启用。
