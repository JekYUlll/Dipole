...
- C3 增加持久化 cutover attempt 状态机：不可变 manifest 绑定源/目标 authority、初始 epoch、中断预算、三阶段节点清单和双组 checkpoint 清单摘要；哈希链事件日志严格约束顺序、时间与外部 artifact 摘要，进程重启后可确定性归约正常续切、冻结期直接回退，以及目标激活后二次冻结回退。日志采用 `0600`、不可覆盖文件与目录 fsync。当前切片只建立恢复决策边界，租约续期、Redis/Kafka 动作执行和自动回切演练仍由 `AD-041` 跟踪。
