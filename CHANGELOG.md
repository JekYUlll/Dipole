...
- 2026-08-31：Read Shadow smoke 增加受控 Core outage 模式：事件进入 EventLedger 后恢复独立 Core，等待 Temporal 重试，并要求 `conversation.list` Step、模型调用和 `conversation_digest` Artifact 绑定完成。该故障注入仅允许在 Read Shadow 隔离项目执行。
