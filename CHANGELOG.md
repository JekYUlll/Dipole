...
- C3 增加默认关闭的共享 delivery fence Go 数据面：版本化 Redis JSON lease 固定 `epoch + authority + active|frozen + expiry`，Gateway 显式启用时在 Kafka/RPC 后续副作用前完成首次核验，并在每条 direct/group message-created 客户端写入或 checkpoint 前重新读取。记录缺失、过期、冻结、epoch/authority 漂移、未知/重复字段及 Redis 故障均 fail closed；被拒 handler 在当前 Kafka record 上等待恢复或进程取消，不进入业务 retry/DLQ。默认配置仍关闭。语言中立 Schema 已冻结，C++ reader、受控 lease writer、节点确认、切换 receipt 和自动回切尚未实现，`AD-041` 继续处理中。
