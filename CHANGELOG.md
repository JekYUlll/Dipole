...
- 本地真实体验验证通过：群摘要使用 DeepSeek 实际检索和二次回答，审批后由 Temporal 按真实时间发布；等待期间重启 Agent 后仍完成原 Task。批准一条、拒绝零条、取消零条，重复批准不增发，WebSocket/Sync 可见。使用现有 smoke，未使用模型、Core 或存储替身；写后异常恢复仍受 AD-008 限制。
