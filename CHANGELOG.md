...
- 2026-08-30：Multipart part 增加 `X-Part-SHA256` 校验链路：现代 Web Crypto 可用时 Web 客户端发送摘要，Core 在保存 ETag/Size 前恒时比较，校验失败不会登记可完成 part；旧客户端缺少该头时保持兼容，并同步更新 Swagger contract 与回归测试。
