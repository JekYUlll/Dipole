...
- 2026-09-01：Agent Runtime 的 Task Timeline 控制响应现在显式将 `bigint` revision、事件序号和时间戳编码为十进制字符串；内部依旧使用 `bigint`，JSON API 与 Gateway 代理不再因 Fastify 无法序列化 `BigInt` 返回 `500`。
