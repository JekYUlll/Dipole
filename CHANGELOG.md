...
- C2 冻结 primary runtime 的 Kafka offset 决策边界：C++ 对完整 `NodeDeliveryBatch/DeliveryAck` 集合复核 batch/delivery identity，仅当所有结果均为 terminal `ENQUEUED|OFFLINE` 时给出 `commit`；`PARTIAL/BACKPRESSURED`、`REJECTED`、`FAILED` 与任何身份/数量漂移均给出 `retain` 或 fail closed。该纯分类器已接入 one-shot transport ACK 校验，尚未授予 `ShadowRunner` Kafka primary 权限。
