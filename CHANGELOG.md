...
- 2026-08-30：Agent Project Guardian 增加版本化低敏 subscription 预筛语料：四类关注项目状态、四类干扰事件、双 reviewer 100% agreement 与共享 deterministic evaluator 已纳入回归。素材固定使用 synthetic `fixture:` 标识，不含真实会话、用户或模型输出；真实 corpus、retrieval relevance、候选模型成本阈值和 shared shadow 观察仍由 `AD-038` 管理。
