...
- 协作总结 `/report`：在同一个 Temporal Task 内完成授权检索、一次缺失信息询问、真实截止时间后继续生成、草稿修改和审批发布。模型各阶段独立持久化，Core Active Run 可保存受鉴权的报告产物。真实 DeepSeek/Temporal/MinIO/IM 验证补充期间重启、两版草稿、批准一条、重复批准不增发、超时后拒绝零条。
- 会话内协作总结面板支持创建、补充、修改、审批和取消；新增直接打开 AI 会话的入口。Pencil 同步维护桌面与移动端设计。任务列表来自当前已加载消息，旧任务需先拉取聊天历史。
