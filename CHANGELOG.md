...
- 2026-09-09：B1 Memory 隔离 smoke 现以受约束的 `DIPOLE_AGENT_MEMORY_B1_CANARY` 参数支持多样 synthetic token，非法值会 fail closed。Remote GPU 的真实 DeepSeek V4 Flash 三样本窗口得到 `2/3` recall 命中，三次 revoke 后的新 Task 均无目标 Memory lineage；`QUARTZ-17` 的未命中以有效 `1/2` Eval（`synthetic_canary_not_recalled`）归档，未被重试或放宽门槛掩盖。详见 [`agent-memory-b1-provider-window-2026-09-09`](benchmarks/agent-memory-b1-provider-window-2026-09-09/)；此 synthetic 窗口不构成真实语义质量或默认持久 Memory 启用结论。
- 2026-09-09：真实 Provider 的隔离 B1 smoke 已可原子写出同一 synthetic canary 的 recall/revoke 两案例 Eval suite。Remote GPU 以 DeepSeek V4 Flash 实跑得到 `2/2` 通过、`10000` bps：recall 为一条 pre-model lineage 且命中 canary，revoke 后的新 Task 为零条该 lineage。低敏 suite 与 CLI report 见 [`agent-memory-b1-synthetic-eval-2026-09-09`](benchmarks/agent-memory-b1-synthetic-eval-2026-09-09/)；revoke 文本仅作观察，Core lineage 仍是撤销读取边界的权威证据。该单一 synthetic 样本不构成真实语义成功率或默认持久 Memory 启用结论。
- 2026-09-09：新增 `eval:memory-b1-synthetic` 多案例 B1 Memory Eval。它仅接受 synthetic canary、哈希化 Task/回复、模型调用数、lineage 数和布尔命中结果；至少两个案例，重复 case/Task hash、调用数或 lineage 漂移均 fail closed。revoke 案例支持 `not_evaluated` 文本状态，避免短期会话历史导致错误的否定文本断言。双案例 fixture、CLI 与类型检查通过；真实 owner-approved 语料与人工语义审核继续独立进行。
- 2026-09-09：B1 Provider smoke 增加 synthetic recall 信号：首条受权回复必须命中已审核的 `ORBIT-91` canary，回执仅记录回复 SHA-256 与布尔结果。Remote GPU 实跑通过，首条为 `1` 次模型调用、`1` 条 lineage 且 canary 命中；revoke 后新 Task 的该 Memory lineage 仍为 `0`。撤销后的回复不作文本否定断言，因为短期会话上下文可能保留历史答案。详见 [`agent-memory-provider-b1-recall-2026-09-09`](benchmarks/agent-memory-provider-b1-recall-2026-09-09/)；单一合成样本不构成成功率或默认启用结论。
- 2026-09-09：B1 Provider smoke 现可选生成低敏、原子落盘的 [`receipt`](benchmarks/agent-memory-provider-b1-2026-09-09/)。回执仅绑定运行时 revision、模型源、哈希化 Task 标识和模型/lineage 计数；未启用 `DIPOLE_AGENT_MEMORY_B1_SMOKE=1` 时传入回执路径会 fail closed。Remote GPU 的真实 Provider 验收确认首条 Task 为 `1` 次模型调用与 `1` 条 lineage，owner revoke 后的新 Task 为 `1` 次模型调用与 `0` 条该 Memory lineage；候选资源已清理，公共体验栈仍为 11 个容器。该证据只覆盖连通性和撤销边界，语义质量与成功率继续待受控 Eval。
- 2026-09-09：Compose profile 明确收口了两处继承开关：只读 MCP shadow 显式关闭 Definition route；交互 MCP 采用独立最终 overlay 恢复其所需的 control/Definition/Artifact/MCP 组合；Memory promotion 显式关闭 interactive control。完整 `scripts/check-compose.sh` 门禁已恢复通过，默认公开 profile 仍不改变。
- 2026-09-09：B1 Memory 隔离 smoke 新增显式 `stub|provider` 模型源。默认 stub 路径保持确定性回归；provider 路径要求可读的 Compose env 文件，并以 `docker compose --env-file` 注入既有凭据，避免 `source`、日志打印或复制密钥。Remote GPU 已以 DeepSeek V4 Flash 完成两条 B1 入站任务：首条的模型调用、非空助手回复和 pre-model Memory lineage 均为一条；owner revoke 后第二条新任务完成模型调用、产生第二条非空回复且该 Memory lineage 为零。候选资源自动清理，公共 `dipole-experience` 保持 11 个容器。该结果验证真实 Provider 的链路连通性与撤销边界，不构成语义质量、成功率或默认持久 Memory 启用结论。

- 2026-09-09：隔离 Interactive Memory Compose profile 的真实 B1 入站验收已通过。版本化候选镜像将同会话、已审核的 canary Memory 经 Core scoped reader 交给 `reply()`；Temporal Task 完成一次模型调用、发送精确一条 `B1_MEMORY_RECALLED_ORBIT_91` 回复，并在模型调用前写入一条 Memory lineage。认证 owner 随后经 Gateway 撤销该 Memory；第二条新入站任务完成一次模型调用、精确回复 `B1_MEMORY_MISSING`，且已撤销 Memory 的 lineage 为零。`dipole_agent` 仅获得 lineage 的 `SELECT/INSERT/UPDATE` 权限；随后既有拒绝与重复批准演练按 B1 后基线通过。候选项目自动清理，公共 `dipole-experience` 保持 11 个健康容器。profile 和默认持久 Memory 均继续关闭，此项使用确定性模型 stub，不代表真实 Provider 质量或默认写入策略已启用。
...
- 2026-09-08：Subscription Active 的隔离 Compose smoke 新增 `publication` 模式：completed shadow Task/Run 通过 Agent Runtime 的 `PromotionEvidencePublisher` 与 mTLS Artifact RPC 发布内容寻址的 promotion evidence，再交由默认关闭的 Gateway proposal/review 完成双人审核和 owner-scoped grant。Remote GPU 验收确认一条 Kafka 订阅事件收敛为一个 completed durable read Task、模型调用存在且 Agent 消息为零；自动清理后候选容器和卷为零，公共 `dipole-experience` 保持 11 个容器。MySQL JSON 列会重排 metadata 键，Artifact Repository 现重新 canonicalize 后再执行 immutable replay 比较，避免成功持久化被误报为冲突。该 smoke 使用合成且合格的 evaluation 样本，只验证发布和控制链路，不代表真实模型质量或生产 promotion 结论。
...
- 2026-09-06：交互任务闭环——Agent 现在能对自建交互任务自主回复（active interactive assistant_reply），体验环境已切到 active interactive 并端到端跑通。
  - Core：`agentActiveApprovedCapabilityProjectionV1` 纳入 `message.assistant_reply.send`；新增自授权原语 `AuthorizeInteractiveReply`（proto RPC + `PersistentAgentApprovalServiceV1.AutoApproveInteractiveReply` + gateway/core 两处 allowlist）：复核任务为 interactive-triggered、pinned Definition 授权 assistant-reply、scope 精确指向 owner 直属 Agent 会话后，铸造一次性已批准 grant。默认仅在 active + `DIPOLE_AGENT_INTERACTIVE_MESSAGE_WRITE_ENABLED=true` 生效。
  - Runtime：新增 `createInteractiveReplyExecutor` 与 `maybeSendInteractiveReply`，规划完成后把 `plan.summary` 作为 `MessageTypeAIText` 投递到 owner 直属会话；`AgentCapabilityRPCClient.authorizeInteractiveReply` 打通 mTLS RPC。修复 `execution-context.ts` 的 `approvedCapabilitySchema` 只收 `message.system.send`，导致 active admission 响应含 `assistant_reply.send` 时 Zod 解析失败、任务卡在 `executeAgentTaskStep`——补 `message.assistant_reply.send` 并加回归测试。
  - 部署：`agent-experience.yml` 由 shadow/read_shadow 切到 `remote`/`interactive_active`（active Kafka group、interactive 队列、interactive message writes 打开、memory 关闭），挂真实 `user_gray` release manifest（`deploy/agent/release-manifest.experience-v1.json`），并为两个 owner Definition 落 v2 promotion grant。
  - 现场验证：LAB113-OPS `dipole-experience` 以新镜像重建 core/agent/gateway，owner `U9927...` 建交互任务 `task:54ed6811...` → `completed`，`message.assistant_reply.send` 审批 `consumed`，直属会话 `direct:U9927...:UAI0001` 落一条 AI 文本回复并同步 2 条 inbox。
  - 边界：回复文本取规划器 `plan.summary`（当前偏"计划性"叙述，后续可换成面向用户的措辞）；promotion grant 用开发期 dummy evidence/eval hash，共享环境正式上线仍需走 operator proposal/review。

- 2026-09-04：修复前端 Artifact 详情页对所有真实响应报 "Artifact metadata 暂不可用" 的契约不一致。
  - 根因:后端 `internal/application/agent_artifact.go::NewAgentArtifactV1` 把 `artifactUUID` 定义为 identity hash(`sha256(schema|task|run|type|version|contentSha256)`),与 `contentSha256`(内容哈希)故意分开,DB `agent_artifacts` 里也是两列(`artifact_uuid` char(64) UNIQUE + `content_sha256` char(64))。而前端 `frontend/src/api/agentArtifacts.ts::parseAgentArtifactMetadata` 早期在两个字段的 64-hex 校验之后又硬加了一句 `if (raw.artifactId !== raw.contentSha256) throw new Error('Agent Artifact content address is invalid')`,并且有配套单测把 `contentSha256: artifactId` 写成 fixture,让契约漂移长期没暴露。真实调用里两个 UUID 必定不等,`AgentArtifactMetadataPanel` 每次都进 `state==='unavailable'` 分支渲染 "Artifact metadata 暂不可用"。
  - 修复:删掉那句非法断言,保留每字段的 64-hex 校验和 `exactKeys` 白名单。同步更新 `agentArtifacts.test.ts` fixture 让 `artifactId !== contentSha256`,并新增一条正向测试断言 "identity hash 与 content hash 独立" 契约,防止再次悄悄回退。
  - 验证:`vitest run src/api/agentArtifacts.test.ts src/components/AgentArtifactMetadata.test.ts src/components/AgentArtifactInbox.test.ts` 12/12 通过;DB 中 `artifact_uuid=0f7e3e60..., content_sha256=7a8544d1...`,Gateway `/api/v1/agent/artifacts/<id>` 已回 200(body_size 503),现在前端能正确渲染详情面板。
...
- 2026-09-04：Agent OAuth callback durable handoff 补上 Runtime 侧的 provider exchange + token lifecycle 组合闭环，默认仍不接线。
  ...
  - 未变更：proto / SQLC / Core Go / Gateway Go / `services/agent-runtime/src/index.ts` / Compose / env / bootstrap / 前端；无新增依赖，`package.json` 不动。`docs/agent/agent-oauth-callback-handoff.md` 加了 `Provider exchange + token lifecycle` 小节，`docs/architecture/ARCHITECTURE-DEBT.md` 同日新增归档条目，`docs/notes/implemented-but-disabled.md` OAuth 行补明本切片仍默认关。回滚：保持该可选组合未装配即可，无入口挂载。
...
- 2026-09-04：默认微服务 Compose 开放只读 Durable Agent Task 控制面。
  ...
  - Remote GPU 在 `a10cea42` 的隔离 Compose project 启动全部默认服务，并通过认证 Gateway 状态路由确认 `taskControlEnabled=true`、`runtimeMode=active`、Temporal `read_active` 与交互消息写入关闭；候选容器和卷已清理。
...
- 2026-09-04：基础微服务 Compose 默认启用 Temporal Durable Agent Runtime。
  ...
  - Remote GPU 的隔离 Compose 已通过启动验收：Agent、Temporal、PostgreSQL 与其余微服务均健康，Temporal Worker 进入 `RUNNING`，Kafka consumer 加入 `dipole-agent-active-primary-v1`；候选容器和卷已清理。
...
- 2026-09-03：Chat 消费 `agent_task_waiting` locator，任务等待审批或输入时提示并链到收件箱。
...
- 2026-09-03：为 Subscription Auto-Reply 补上 activity 重试幂等,收尾 AD-034 最后一项。
  ...
  - 语义：自主回复变成幂等 + exactly-once(happy path)/at-most-once(消费后 send 恰好瞬断且始终无法完成的极端窗口不再重驱,但也不再无限重试拖垮 Task),对「无人值守最佳努力回复」是合理边界。happy path 字节级不变,故既有 Remote smoke 仍具代表性。
  - 验证：新增 3 条投影单测(首发、已交付重试跳过不双发、真错 rethrow),agent-runtime `typecheck` 与定向单测(投影 + events + read activity 32 项)本地绿;全套 856 项通过(同一处 external-mcp 凭据文件属主用例在 root 环境不适用)。AD-034 的 durable-step 幂等项收尾。

- 2026-09-02：Subscription Auto-Reply 端到端 smoke 打通，顺带修了两个真 bug。
  - 新增 `scripts/smoke-agent-subscription-autoreply-compose.sh`：隔离 Compose 里 owner 发一条消息触发订阅，验证「一个事件 → 一个 Task → 一次模型调用 → 一条自主回复」，回复经 Core `AuthorizeSubscriptionMessage` 铸并消费一次 `message.system.send` grant。
  - Bug 1：Core 的 caller allowlist（`internal/services/core/rpcpolicy/policy.go`、`internal/transport/grpc/core/policy.go`）漏了 `AuthorizeSubscriptionMessage`，agent 调用被 `PERMISSION_DENIED`。已补白名单 + 正例断言。
  - Bug 2：自触发死循环。自主回复以 system 消息写回被订阅的会话，而 `services/agent-runtime/src/events/shadow-processor.ts` 的自我抑制只认 `lineage.origin=agent`，system 消息漏网，回复反复触发订阅（一次触发跑出上百个 Task）。改为按 `sender_uuid == agentUuid` 抑制 Agent 自己发的任何消息，命令 kind 无关。
  - Remote GPU 用独立 tag 镜像（不动 `:latest` 的在用调试栈）跑 smoke 绿：恰好一条消息、一次 grant 消费、一次 tool invocation、无循环。Go policy 包 `vet`/`test`、agent-runtime `typecheck` 与 events 单测本地通过。AD-034 的 Remote smoke 项已关闭；durable step 幂等仍开。

- 2026-09-02：为 Subscription Auto-Reply 增加默认关的 Compose opt-in overlay 与静态门禁。新增 `deploy/microservices/agent-subscription-autoreply.yml`,叠加在 `agent-active.yml + agent-subscription-active.yml` 之上,**只**把 `DIPOLE_AGENT_SUBSCRIPTION_MESSAGE_WRITE_ENABLED` 翻成 `true`;subscription trigger、独立 Kafka group/Temporal 队列、指向 Core 的 Capability RPC(`core:9091`)全部从下层继承,不引入任何其他能力开关。同时把 `agent-subscription-active.yml` 中该 flag 显式固定为 `false`(订阅只读闭环默认不发消息)。`scripts/check-compose.sh` 扩展:①订阅只读 profile 断言该 flag `== "false"`;②新增 `subscription_autoreply_config` 渲染断言该 flag `== "true"` 且 Capability RPC 指向 `core:9091`、交互写仍 `false`、Control/MCP/Gateway 面保持关闭。新增静态契约测试 `scripts/test_agent_subscription_autoreply_compose.py`(overlay 仅翻写 flag、只读 profile 默认关、门禁双向覆盖),与既有 5 项 compose 契约测试通过;`bash -n` 语法校验通过。`docker compose` v2 在本机不可用,完整 `check-compose.sh` 渲染门禁待 Remote GPU/CI 复核;本地以等价 env-merge 校验 overlay 生效值。运行时 refinement(该 flag 必须与 `subscription_active` 同开且与交互写互斥)在上一切片已落地。剩余 Remote smoke(`agent_sent_messages=1`、审批消费一次、重放幂等)与 durable step 幂等继续由 `AD-034` 跟踪。

- 2026-09-02：为 Subscription Auto-Reply 落地 Core 侧订阅 scoped 自主审批(默认关,自主回复仍未启用)。新增 Agent Capability RPC `AuthorizeSubscriptionMessage`(复用 `RequestApprovalRequest`/`ApprovalResponse`),在回复内容定案后由 Core 校验四项不变量后**直接铸造** `status=approved`、`approved_by=principal` 的写 grant,替代 owner 手动 Signal:①Task 为订阅触发(`trigger_subscription_uuid` 非空);②pinned Definition 仍 owner 一致且投影出 `message.system.send`;③订阅 `created_by` 与 Task principal 一致;④scope 恰为 owner direct Agent 会话(`conversation`/`direct:<owner>:<agent>`/`write`,按 `dipole.agent.scope.v1` 校验 `scope_sha256`)。落 grant 复用 `InsertAgentApproval`,对相同绑定幂等(重放收敛到同一 grant,绑定漂移拒绝)。TS 侧 `createSubscriptionMessageExecutor` 在发送前先调用该 RPC 铸 grant,再走既有 resolve/consume 写链路;binding 由 `eventId`+`occurredAtUnixMs`+回复正文确定性派生(订阅专用 `dipole.agent.subscription-message.v1`/`.nonce.v1` 命名空间),使重试铸出同一 grant。RPC 由 caller `dipole-agent`、空 principal、`active` 模式门控,并写入审批时间线。agent-runtime `typecheck` 与定向单测(投影铸 grant + 独立命名空间、activity 订阅回复正/负例)通过;Go 侧 service 正/负例(幂等、非订阅、owner 不一致、scope/capability 不匹配、过期)与 server 接线测试待 Remote GPU 复核(proto Go/TS stub 再生 + `go build/vet/test` + MySQL 契约)。**注意**:内联"铸 grant→resolve→consume→send"因 consume 一次性,与 Temporal activity 重试的幂等仍需把回复做成独立带 checkpoint 的 durable step,连同 compose overlay(默认关)+静态门禁与 Remote smoke(`agent_sent_messages=1`、审批消费一次、重放幂等)继续由 `AD-034` 跟踪。

- 2026-09-02：为 Subscription Auto-Reply 接线运行时回复执行步骤(仍默认关)。订阅触发的 active read 完成时,若 `subscriptionMessageWritesEnabled` 已把 `subscriptionMessage` 执行器接入 Temporal read activity,则把模型摘要投递到 owner 的 direct Agent 会话(`direct:<owner>:<agent>`),复用与交互写完全相同的审批绑定审计写链路(capability `message.system.send`、`system_message`);仅在 `active` 模式、事件带 `subscriptionId` 且摘要非空时触发,并使用独立的 Tool Invocation 命名空间(`dipole.agent.subscription-message-invocation.v1`)保证与交互写在同一 Task 内不碰撞。回复复用审批闸,授权仍按 `(task,run,capability,scope,argumentsSha256)` 解析 grant——因自主回复无 owner 手动 Signal,该 grant 需由 Core 在**回复内容定案后**(而非订阅 Task 准入时:`argumentsSha256` 依赖回复正文,准入时内容未知)经订阅 scoped 自主审批直接创建 approved grant,作为后续切片。选择投递到 owner Agent 会话而非触发会话,避免引入向人-人会话写入的更大授权面。agent-runtime `typecheck`、`build` 与定向单测(消息投影独立命名空间、activity 订阅回复正/负用例)通过,全套 853 项通过(一处 external-mcp 凭据文件属主用例在 root 运行环境下不适用)。后续 Core 自动审批、compose overlay(默认关)+静态门禁与 Remote smoke(断言 `agent_sent_messages=1` 且幂等)继续由 `AD-034` 跟踪。
...
- 2026-09-02：设计源 `design/dipole-ui.pen` 的帧内联色统一到 V3，与前端 token、品牌板对齐，完成前端与设计治理的收口。新增确定性脚本 `scripts/pen-v3-recolor.mjs`：把两代并存的变量单代化（363 处遗留变量引用重指向 V3 语义变量后删除 6 个遗留变量，新增 `success`/`success-soft` 对齐前端 `--dp-success`），并把 240 处内联 fill/stroke 旧盘 hex（64 种）按角色映射到 V3 变量——结构=海军蓝、身份/进度=金、正向=BI 绿、注意=琥珀、销毁=danger，鼠尾草/电报蓝中性折叠到暖象牙 `line`/`ink` 阶，纯白/纯黑保留；脚本幂等，带残留旧盘 hex 与悬空引用断言。`.pen` 变量 38→34，`check-pencil-design`、`test:design`、`test:brand` 与 180 项前端测试全绿，pen 无头渲染抽查 Foundations/Login/Chat/Agent 板视觉确认为 V3。`design/exports/` 已批准评审 PNG 仍是改色前快照——无头 pen CLI 只出 400px 缩略图、无缩放，不足以复现已批准的全分辨率基线，故不降质覆盖，留待 pen.dev 桌面应用按批准分辨率重出（纯渲染、无配色决策，见 ARCHITECTURE-DEBT）。仅设计源与文档改动，不影响运行时与服务 authority。
...
- 2026-09-09：修正并行 `agent-subscription` experience overlay 的 release manifest 绑定。跨文件 Compose `extends` 会继承基础开发 manifest，现改为强制提供与 candidate 匹配的 `DIPOLE_AGENT_RELEASE_MANIFEST_FILE`，缺失时在配置阶段拒绝。Remote GPU 已以公共 experience manifest 启动独立只读订阅 worker；其健康运行且 interactive Agent 保持零次重启。
- 2026-09-09：公共 `agent-subscription` worker 已完成首个只读 Subscription Active Read 验收。修正跨文件 `extends` 漏继承 experience 模型预算后，新的 owner Definition/群 Subscription 在短期 fixture grant 下经 Kafka、Temporal 和 Core 收敛为 `completed:completed`，完成两次模型调用且零条 Agent 群消息；fixture grant 在脚本退出时撤销。新增可复跑 `scripts/e2e-agent-subscription-active-read.sh`，B1/B2 体验脚本继续保留。
- 2026-09-09：Remote GPU 以当前 `master` 镜像重跑隔离 `subscription_active` control smoke：Gateway proposal、独立 reviewer、owner-scoped grant 与一条 Kafka 订阅事件完整收敛为 completed read Task，存在模型调用且零 Agent 消息；隔离容器和卷自动清理，公共体验 12 服务未重启。Smoke 的 MySQL 调用改为容器内 `MYSQL_PWD`，移除密码命令行告警。
...
- 2026-09-09：修正共享 promotion window 的 Gateway 重建健康轮询。`close` 现在明确等待短暂的 `starting` 状态收敛为 `healthy` 后才判定关闭结果；Remote GPU 已完成一次 B1 真实入站私聊闭环，并随后恢复 promotion route 为关闭。该窗口未签发 Runtime promotion grant，未启用订阅自动回复、MCP 写入或 Memory 默认写入。
- 2026-09-09：External MCP durable elicitation 支持最多 8 轮连续 form 输入。每轮以 Core 持久 round receipt、轮次派生 Request ID 和 SHA-256 checkpoint 绑定，恢复时重新解析权威命令并重放相同 Invocation；Temporal 两轮 `waiting_input`、worker replacement 后完成的集成验收已通过。默认 external MCP route、URL mode 与敏感输入继续关闭。
- 2026-09-09：Temporal Task client 现将运行中 workflow 的 `WorkflowExecutionAlreadyStartedError` 收敛为稳定 workflow ID，避免重复入站调度将合法并发误报为失败；已关闭 execution 仍遵循 `ALLOW_DUPLICATE` 创建新 run。15 项 Task workflow Temporal 集成验收恢复通过，并修正恢复 fixture 以 stage-scoped model receipt 验证 plan/synthesis 重放无额外调用。
- 2026-09-09：Multipart Core 新增服务端路径归因的终态指标 `dipole_multipart_upload_terminal_total`。Redis session 在预签名 URL 签发和 relay 回退后持久化标记，完成上传可区分 `direct`、`direct_fallback` 与 `relay`，中止同样只产生一次终态；指标保持低基数且不含用户、session、文件或对象标识。默认上传模式仍为 `relay`，该指标只是 24 小时预签名观测 receipt 的输入之一。
- 2026-09-09：Remote GPU 公共 `dipole-experience` 的 Core 已滚动至 `b527af52` 并保持 healthy。一次性开发用户完成 4-byte relay Multipart 上传，Core `/metrics` 返回 `dipole_multipart_upload_terminal_total{outcome="completed",route="relay"}`；Route A 与默认 `relay` 上传策略保持关闭/不变。该单样本仅验证运行时指标接线，不构成预签名切流 evidence。
