...
- C3 增加持久化 cutover attempt 状态机与单步恢复 orchestrator：不可变 manifest 绑定源/目标 authority、初始 epoch、中断预算、三阶段节点清单和双组 checkpoint 清单摘要；哈希链事件日志严格约束顺序、时间与外部 artifact 摘要。每一步使用确定性 action ID，执行失败不推进 journal，替换进程会重试同一动作；首次 freeze 超过中断预算后自动转入源 authority 回退。正常续切、冻结期直接回退，以及目标激活后二次冻结回退均有确定性归约。日志采用 `0600`、不可覆盖文件与目录 fsync。生产 Redis/Kafka executor、租约续期和自动回切演练仍由 `AD-041` 跟踪。
