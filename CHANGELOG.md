...
- 将 Core RPC 组合逻辑迁入 `internal/services/core/rpc/`；embedded runtime 直接使用 Core-owned composition，`internal/bootstrap/internal_rpc.go` 收缩为迁移期兼容 wrapper，RPC 协议、mTLS、caller allowlist 和 Agent 方法权限保持兼容。
