...
- MCP Message write projection 没有数据迁移或生产开关。`createDipoleMcpServer` 只有同时收到 active ExecutionContext、审批必需的 write descriptor、Command kind 和显式 executor 才注册写 Tool；当前启动链不提供这些条件。未来启用前还需实现受信 grant resolver、active authority 晋级、UI 风险摘要，并演练 Command RPC 超时后通过 receipt 收敛 action lineage。
