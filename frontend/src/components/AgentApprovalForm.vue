<template>
  <section class="approval-shell" :class="{ 'is-embedded': embedded }" :data-agent-approval-state="viewState" :aria-busy="busy" aria-live="polite">
...
const props = withDefaults(defineProps<{ taskId: string; client?: AgentTaskClient; now?: () => number; embedded?: boolean }>(), {
...
<style scoped>
.approval-shell { --ink:var(--dp-ink); --muted:var(--dp-ink-soft); --paper:var(--dp-canvas); --cream:var(--dp-surface); --line:var(--dp-line); --accent:var(--dp-danger); --forest:var(--dp-rail); min-height:100vh; padding:clamp(24px,5vw,64px); color:var(--ink); background:radial-gradient(circle at 82% 4%,var(--dp-danger-soft),transparent 27%),repeating-linear-gradient(90deg,transparent 0 79px,color-mix(in srgb,var(--dp-rail) 4%,transparent) 80px),var(--paper); font-family:var(--dp-font-body) }
.approval-header { max-width:920px; margin:0 auto 24px; display:flex; align-items:end; justify-content:space-between; gap:24px }.approval-header h1 { margin:4px 0 0; font-family:var(--dp-font-display); font-size:clamp(30px,5vw,52px); font-weight:500; letter-spacing:-.04em }.eyebrow,.rail-label { margin:0; color:var(--accent); font-size:11px; font-weight:800; letter-spacing:.16em }.task-badge { padding:8px 11px; border:1px solid var(--line); border-radius:999px; color:var(--muted); background:color-mix(in srgb,var(--dp-surface) 45%,transparent); font:700 11px var(--dp-font-data) }
.approval-grid { max-width:920px; margin:0 auto; display:grid; grid-template-columns:minmax(220px,290px) 1fr; overflow:hidden; border:1px solid color-mix(in srgb,var(--ink) 16%,transparent); border-radius:6px 24px 6px 6px; box-shadow:0 26px 70px color-mix(in srgb,var(--dp-rail) 14%,transparent) }.source-rail { padding:32px; color:var(--dp-text-inverse); background:var(--forest) }.source-rail .rail-label { color:var(--dp-agent-soft) }.source-rail dl { margin:24px 0 0 }.source-rail dt { margin-top:18px; color:var(--dp-ink-faint); font-size:10px; text-transform:uppercase; letter-spacing:.12em }.source-rail dd { margin:4px 0 0; overflow-wrap:anywhere; font:600 12px var(--dp-font-data) }.deadline { margin-top:36px; padding-top:20px; border-top:1px solid color-mix(in srgb,var(--dp-text-inverse) 14%,transparent); display:flex; justify-content:space-between; color:var(--dp-ink-faint); font-size:11px }.deadline strong { color:var(--dp-text-inverse) }.approval-panel { padding:clamp(28px,5vw,56px); background:var(--cream) }.approval-panel h2 { margin:12px 0 18px; font-family:var(--dp-font-display); font-size:clamp(22px,3vw,34px); line-height:1.22; font-weight:500 }.warning { margin:0; padding:14px 16px; border-left:4px solid var(--accent); color:var(--muted); background:var(--dp-danger-soft); font-size:13px; line-height:1.65 }.action-row { display:flex; justify-content:flex-end; gap:10px; margin-top:38px; padding-top:24px; border-top:1px solid var(--line) }button,.link-button { min-height:42px; padding:0 17px; border-radius:var(--dp-radius-sm); font:750 12px inherit; cursor:pointer }button:focus-visible,.link-button:focus-visible { outline:3px solid color-mix(in srgb,var(--dp-danger) 35%,transparent); outline-offset:3px }.primary-button { border:1px solid var(--accent); color:var(--dp-text-inverse); background:var(--accent) }.secondary-button { border:1px solid var(--line); color:var(--ink); background:transparent }button:disabled { cursor:not-allowed; opacity:.55 }.state-card { max-width:760px; margin:80px auto; padding:34px; border:1px solid var(--line); border-radius:4px 18px 4px 4px; background:var(--cream); box-shadow:0 20px 55px color-mix(in srgb,var(--dp-rail) 11%,transparent) }.state-card-danger { border-left:5px solid var(--accent) }.state-card-complete { border-left:5px solid var(--dp-agent) }.state-title { margin:0 0 8px; font-family:var(--dp-font-display); font-size:22px }.state-card>p:not(.state-title) { color:var(--muted) }.state-card .secondary-button { margin-top:12px }.link-button { display:inline-flex; align-items:center; text-decoration:none }
...
.approval-shell.is-embedded { min-height:auto; padding:0; background:transparent; }
.approval-shell.is-embedded .approval-grid { grid-template-columns:1fr; max-width:none; box-shadow:none; border-radius:0; }
.approval-shell.is-embedded .approval-header { display:none; }
</style>
