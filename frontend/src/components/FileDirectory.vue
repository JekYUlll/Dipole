<template>
  <section class="file-directory" :data-file-state="state" :aria-busy="state === 'loading'">
    <aside class="file-rail" aria-label="Primary navigation">
      <strong>DIPOLE</strong><span>OWNER WORKSPACE</span>
      <nav><span>会话</span><span>联系人</span><span>群组</span><b>文件</b><span>设备</span></nav>
      ...
      <template v-else>
        ...
        <div class="file-list">
          <article v-for="file in files" :key="file.file_id" class="file-card">
            <div class="file-icon" aria-hidden="true">{{ extension(file) }}</div>
            <div class="identity"><h3>{{ file.file_name }}</h3><p>{{ formatBytes(file.file_size) }} · {{ formatTime(file.created_at) }}</p><small>{{ file.content_type || 'application/octet-stream' }}</small></div>
            <button class="download" :disabled="downloading === file.file_id" @click="download(file)">{{ downloading === file.file_id ? '准备中' : '下载' }}</button>
          </article>
        </div>
...
async function load() {
  state.value = 'loading'; files.value = []; nextCursor.value = undefined; hasMore.value = false
  try {
    const page = await props.client.list()
    files.value = page.files; nextCursor.value = page.next_cursor; hasMore.value = page.has_more; state.value = 'ready'
...
async function loadMore() {
  ...
  try {
    const page = await props.client.list(nextCursor.value)
    files.value = [...files.value, ...page.files]; nextCursor.value = page.next_cursor; hasMore.value = page.has_more
  } catch { files.value = []; nextCursor.value = undefined; hasMore.value = false; state.value = 'unavailable' }
  finally { loadingMore.value = false }
}

async function download(file: OwnedFileDirectoryItem) {
  downloading.value = file.file_id
  try { window.open(await props.client.download(file.file_id), '_blank', 'noopener,noreferrer') }
  finally { downloading.value = undefined }
}
