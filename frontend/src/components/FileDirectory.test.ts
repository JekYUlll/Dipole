...
const item = {
  file_id: 'F100', file_name: 'handoff.md', file_size: 42, content_type: 'text/markdown',
  created_at: '2026-08-30T00:00:00.000Z', download_path: '/api/v1/files/F100/download',
}

describe('FileDirectory', () => {
  it('renders only the owner-safe projection and opens a newly authorized download', async () => {
    const open = vi.spyOn(window, 'open').mockImplementation(() => null)
    const client = { list: vi.fn(async () => ({ files: [item], has_more: false })), download: vi.fn(async () => 'https://signed.example/F100') }
    const wrapper = mount(FileDirectory, { props: { client } })
    await flushPromises()
