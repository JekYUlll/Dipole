<template>
  <section class="settings-shell">
    <aside class="settings-rail" aria-label="Primary navigation">
      <img class="brand-logo" :src="dipoleLogo" alt="Dipole IM" />
      <span>ACCOUNT / CLIENT</span>
      <nav><RouterLink :to="{ name: 'chat' }">会话</RouterLink><RouterLink :to="{ name: 'contacts' }">联系人</RouterLink><RouterLink :to="{ name: 'groups' }">群组</RouterLink><RouterLink :to="{ name: 'files' }">文件</RouterLink><RouterLink :to="{ name: 'devices' }">设备</RouterLink><RouterLink class="active" :to="{ name: 'settings' }">设置</RouterLink></nav>
      <p>OWNER SCOPED<br />LOCAL CLIENT CONTROL</p>
    </aside>
    <main class="settings-page" aria-labelledby="settings-title">
      <div class="mobile-brandbar"><img :src="dipoleLogo" alt="Dipole IM" /><span>账户设置</span><RouterLink :to="{ name: 'chat' }">返回会话</RouterLink></div>
    ...
    <section class="settings-card" aria-labelledby="device-title">
      ...
      <p class="card-copy">设备管理使用独立的隐私安全页面，仅披露设备名称、粗粒度类型和活动时间。</p>
    </section>
...
<style scoped>
.settings-shell{min-height:100vh;background:var(--dp-canvas);color:var(--dp-ink);display:grid;grid-template-columns:256px 1fr;font-family:var(--dp-font-body)}.settings-rail{background:var(--dp-rail);color:var(--dp-text-inverse);padding:34px 26px;display:flex;flex-direction:column;gap:12px}.brand-logo{display:block;width:116px;height:86px;object-fit:contain;object-position:center;margin:-8px 0}.settings-rail>span,.settings-rail p{font:10px var(--dp-font-data);letter-spacing:.1em;color:var(--dp-ink-faint)}.settings-rail nav{display:grid;gap:7px;margin-top:24px}.settings-rail nav a{padding:11px 13px;border-radius:var(--dp-radius-sm);font-size:13px;color:var(--dp-ink-faint);text-decoration:none}.settings-rail nav .active{background:var(--dp-rail-soft);color:var(--dp-text-inverse)}.settings-rail p{margin-top:auto;line-height:1.8}.settings-page{min-height:100vh;max-width:960px;width:100%;margin:auto;box-sizing:border-box;padding:48px 54px 68px}.settings-header,.card-heading,.actions,.logout-card{display:flex;align-items:center;justify-content:space-between;gap:20px}.settings-header{margin-bottom:28px}.settings-header h1{font:800 38px var(--dp-font-display);letter-spacing:-.04em;margin:6px 0}.settings-header>div>p:last-child,.card-copy{color:var(--dp-ink-soft);margin:0;line-height:1.7}.eyebrow{color:var(--dp-accent-strong);font:700 10px var(--dp-font-data);letter-spacing:.12em;margin:0}.back-link,.secondary-link{color:var(--dp-accent-strong);font-weight:700;text-decoration:none}.settings-card{background:var(--dp-surface);border:1px solid var(--dp-line);border-radius:var(--dp-radius-md);box-shadow:0 12px 30px #17211d0d;margin-top:16px;padding:28px}.settings-card h2{font:800 21px var(--dp-font-display);margin:5px 0 0}.identity,.sync-state{color:var(--dp-ink-soft);font:600 12px var(--dp-font-data)}.signature-field{display:grid;gap:8px;font-weight:700;margin-top:22px}.signature-field textarea{border:1px solid var(--dp-line);border-radius:var(--dp-radius-sm);font:inherit;min-height:92px;padding:12px;resize:vertical}.signature-field small{color:var(--dp-ink-faint);text-align:right}.actions{justify-content:flex-end;margin-top:16px}.primary,.danger{border:0;border-radius:var(--dp-radius-sm);color:var(--dp-text-inverse);cursor:pointer;font:700 13px var(--dp-font-body);padding:11px 16px}.primary{background:var(--dp-accent-strong)}.danger{background:var(--dp-danger)}button:disabled{cursor:not-allowed;opacity:.6}.sync-details{display:grid;gap:12px;grid-template-columns:repeat(2,minmax(0,1fr));margin:20px 0 0}.sync-details div{background:var(--dp-surface-muted);border-radius:var(--dp-radius-sm);padding:15px}.sync-details dt{color:var(--dp-ink-soft);font-size:12px}.sync-details dd{font:700 15px var(--dp-font-data);margin:6px 0 0}.notice{border-radius:var(--dp-radius-sm);display:inline-flex;gap:8px;margin:0 0 16px;padding:10px 12px}.notice button{background:transparent;border:0;color:inherit;cursor:pointer;font-weight:700;text-decoration:underline}.error{background:var(--dp-danger-soft);color:var(--dp-danger)}.success{background:var(--dp-accent-soft);color:var(--dp-accent-strong);margin:0}.logout-card{margin-bottom:40px}.mobile-brandbar{display:none}@media(max-width:760px){.settings-shell{display:block}.settings-rail{display:none}.settings-page{padding:28px 18px 48px}.settings-header{align-items:flex-start}.settings-header h1{font-size:31px}.settings-card{padding:20px}.card-heading,.logout-card{align-items:flex-start;flex-direction:column}.sync-details{grid-template-columns:1fr}.actions{align-items:stretch;flex-direction:column}.primary,.danger{width:100%}.mobile-brandbar{display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--dp-line);padding:0 0 17px;margin-bottom:28px}.mobile-brandbar img{width:48px;height:36px;object-fit:contain}.mobile-brandbar span{font:800 15px var(--dp-font-display);flex:1}.mobile-brandbar a{color:var(--dp-accent-strong);font-size:12px;font-weight:700;text-decoration:none}}
</style>
