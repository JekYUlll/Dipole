...
export function parseOwnedFileDirectoryPage(raw: unknown): OwnedFileDirectoryPage {
  ...
  return { files: raw.files.map(parseItem), ...(raw.next_cursor ? { next_cursor: raw.next_cursor } : {}), has_more: raw.has_more }
}
...
export const ownedFileDirectoryClient: OwnedFileDirectoryClient = {
  ...
  async download(fileID: string) {
    ...
    if (!isRecord(raw) || typeof raw.download_url !== 'string' || raw.download_url.trim() === '') throw new Error('file download response is invalid')
...
function parseItem(raw: unknown): OwnedFileDirectoryItem {
  if (!isRecord(raw) || !exactKeys(raw, itemKeys) || typeof raw.file_id !== 'string' || !identifier.test(raw.file_id) ||
      typeof raw.file_name !== 'string' || raw.file_name.trim() === '' || raw.file_name.length > 255 || typeof raw.file_size !== 'number' ||
      !Number.isSafeInteger(raw.file_size) || raw.file_size < 0 || typeof raw.content_type !== 'string' || raw.content_type.length > 255 ||
  ...
  return raw as unknown as OwnedFileDirectoryItem
}
