...
`McpMessageWriteProjection` 将上述边界组合成默认关闭的第一方写路径。Server 仅在 ExecutionContext 为 active、Capability 明确要求审批并注入 write executor 时接受写投影；projection 在消费审批前限制目标为当前 principal 与 Agent 的 direct conversation，随后把 Tool runner 生成的 Invocation ID 传入 Command RPC，并将返回引用绑定到成功终态。Core grant resolver 与 TS adapter 已具备。MCP context 会采用 Core 从持久 Run 返回的权威 `runtime_id/mode`，两端都校验固定 Runtime identity；active Run admission 还必须经过绑定 Runtime candidate、Task 与 pinned Definition 的注入式 promotion authorizer。生产未注入 authorizer，`index.ts` 仍只注册 `conversation.list` 且没有 write executor。上线前还需持久 promotion grant、approved-capability 投影，以及 RPC deadline/cancellation 发生在服务端提交之后的 receipt 与 action-lineage 收敛演练。
