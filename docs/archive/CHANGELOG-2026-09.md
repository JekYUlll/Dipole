...
- 建立面向简历与面试的文档治理入口：新增 `docs/guides/PROJECT-LEARNING-AND-INTERVIEW.md` 总索引，并将 Dipole IM、Dipole Agent、Go/Eino 与跨组件技术问答拆分维护；同步标注 `verified`、`shadow`、`candidate`、`default-off` 和 `planned` 证据状态，避免把 legacy、候选或未来能力写成默认生产事实。

- 新增 authenticated Device Directory：读取当前账户设备会话，支持单设备下线、全部下线、不可用清空重试，以及 V3 desktop/mobile Chromium 视觉基线；会话详情保持 owner-scoped。

- V3 品牌追踪流程已增加颜色量化与二值 alpha mask 预处理，再交由 VTracer 生成路径；重新生成的 IM/Agent SVG 去除栅格边缘残影，保留透明通道和原始图形构图。`scripts/trace-brand-assets.sh` 仍是唯一可复现入口。

- 新增 authenticated Group Directory：从当前账户可见的群组会话投影读取群详情，支持热群 `notify + pull` 状态、解散群只读提示、不可用清空重试，以及 V3 desktop/mobile Chromium 视觉基线；群组写入操作保持关闭。

- 品牌资产转换流程已固定为基于 `LOGO_V3.png` 的 VTracer 追踪，新增独立的 IM 紧凑 mark；完整锁定稿与紧凑标志均可由 `scripts/trace-brand-assets.sh` 重现，避免手工重绘造成视觉偏差。

- 新增 authenticated Contact Directory：从 `/api/v1/contacts` 读取当前账户可见的联系人投影，提供空态、不可用重试、移动端品牌栏和 V3 Chromium desktop/mobile 视觉基线；页面保持只读，不提前开放关系写入动作。

- V3 品牌 SVG 已按原始 PNG 的完整可见区域重新追踪：移除画布底色、补齐 Agent 左侧轨道，并使用更高颜色精度和更小简化阈值保留图形边缘、轨道节点与字标细节；脚本已固定裁剪区域和生成参数。

- Agent Definition Catalog 已新增受保护的 owner-scoped 只读页面，展示 Core 返回的 active Definition、版本、会话 scope 和有效期；新增桌面/移动 Chromium 视觉基线与 Core 不可用时清空旧数据的测试，入口由 `VITE_AGENT_DEFINITIONS_ENABLED` 默认关闭。

- Agent Approval 与 Elicitation 操作表单已补齐 390x844 移动端 Chromium 视觉基线；验证单列布局、按钮顺序、MCP 来源披露和截止时间区域与 V3 设计矩阵一致，Firefox/WebKit 继续按 canonical Chromium 规则跳过像素断言。

- V3 品牌资产已按 `LOGO_V3.png` 的实际裁剪区域重新用 VTracer 生成：IM 锁定稿、Agent 完整锁定稿和 Agent 紧凑标志均保留原图的双极图形、轨道与橙色节点；新增 `scripts/trace-brand-assets.sh` 固定裁剪坐标和追踪参数，便于后续复现与审阅。

- Chat Desktop/Mobile 已完成首轮 V3 主题收口：导航 rail、会话侧栏、空状态、消息气泡、输入区和详情区统一接入 Pencil token 层；新增受控 Chromium Desktop/Mobile 视觉场景与截图基线，前端全量 `28` 个测试文件、`108` 项通过，并已部署到 Remote GPU 候选 `b09c7fa2`。

- Agent Memory 与 Event Subscription 控制台已改用从 `LOGO_V3.png` 经 VTracer 追踪生成的紧凑 SVG 标记；新增 `dipole-v3-agent-mark-traced.svg`，并通过组件契约测试与生产构建验证。

- Chat 页面 V3 视觉漂移已修复：移除遗留硬编码 `#07c160`，统一使用 `--dp-accent`；新增 ChatView 设计契约测试，前端全量 27 个测试文件、103 项通过，并已部署到 Remote GPU 候选 `3ca006e0`。

- 微服务 Compose 的 Gateway 宿主端口改为支持 `DIPOLE_GATEWAY_PORT` 覆盖，默认仍为 `8080`；Remote GPU 候选可移除临时端口 overlay，使用 `DIPOLE_GATEWAY_PORT=18097` 复用同一份正式配置。

- Remote GPU 隔离候选已切换至 `9f416710`：Core、Gateway、Agent 镜像 revision 一致，Gateway 健康检查通过，公网 `http://223.111.157.214:18097/app/` 可访问，VTracer 生成的 IM SVG 以 `image/svg+xml` 返回，Agent `/readyz` 返回 200。该候选使用远程临时 Compose 端口 overlay 保持 `18097`，未修改宿主机网络或 Docker daemon。

- 前端 V3 视觉回归基线已刷新：覆盖 Agent Elicitation、Artifact、Definition Catalog、Governance、Task Timeline、Chat、File Directory 和 Settings 共 9 个 Chromium 快照；Chromium 验收 40 项通过、2 项按测试设计跳过，Firefox 与 WebKit 独立验收均为 18 项通过、12 项按测试设计跳过。此前 Firefox 与 Chromium 并行时的连接拒绝归因于固定本地端口竞争，未作为产品缺陷记录。

- 移除遗留单色蓝色 PNG 主标，README 与文档入口统一使用深青色 Signal SVG 视觉系统；双极与橙色事件脉冲继续表达实时协作链路。本项只影响品牌呈现。

- Remote GPU 真实 MinIO Multipart restart smoke 通过：首个分片写入后重启带持久卷的 MinIO，续传并 Complete 后最终对象内容一致；隔离容器和数据卷已自动清理，未触碰 GPU 任务或业务容器。浏览器断网、过期会话、网关限流和跨存储故障矩阵仍待完成。
