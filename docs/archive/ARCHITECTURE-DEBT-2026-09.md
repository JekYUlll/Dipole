...
- 2026-09-08：新增面向简历与面试的主文档及 IM/Agent/技术参考分册，明确 Go/Eino baseline、TypeScript Runtime、shadow/candidate/default-off 状态和代码证据入口；后续架构切片需同步更新对应分册。
...
### 2026-09-01：V3 前端候选已完成远程部署验证

- Remote GPU 隔离候选已固定到 `9f4167101f8d9631bd365834efc880b094838150`；Core、Gateway、Agent 镜像 revision 一致，Gateway 与 Agent readiness 均通过。
- 公网 `http://223.111.157.214:18097/app/` 和 VTracer 追踪版 SVG 资源均已验证可访问；部署使用仅存在于远程 `/tmp` 的端口 overlay，将基础 Compose 的 `8080` 映射替换为 `18097`。
- 后续应将端口参数化和 Remote GPU overlay 纳入正式部署脚本，并在当前候选停止前保留可回滚 revision；本次未修改宿主机网络、Docker daemon 或其他 Compose 项目。

### 2026-09-01：V3 前端跨浏览器验收边界

- Chromium 视觉回归基线已刷新并复跑通过，结果为 40 项通过、2 项按测试设计跳过；Firefox 独立验收为 18 项通过、12 项按测试设计跳过。
- Firefox 曾与 Chromium 并行运行并竞争固定的 `127.0.0.1:4173` 测试服务，产生的连接拒绝已通过串行复跑消除，不归因于产品行为。
- WebKit 独立验收已完成，为 18 项通过、12 项按测试设计跳过；当前证据覆盖 Chromium、Firefox 和 WebKit，但仍需在远程部署环境补充真实网络与设备矩阵。
- Chat V3 token 漂移已收敛到共享 `--dp-accent`，并在 Remote GPU 候选 `3ca006e0` 完成公网页面、SVG MIME、Gateway health 和隔离 Compose 状态复核。
...
### AD-054：Multipart 真实故障矩阵仍未完整关闭

- **优先级：** P1
- **状态：** 进行中
- **发现日期：** 2026-08-30
- **影响范围：** 大文件上传恢复、对象存储资源回收、预签名切流和网关限流验收
- **现状：** Remote GPU 已通过隔离 MinIO 基础生命周期与服务重启恢复 smoke；客户端浏览器断网、过期会话、网关限流、代理超时及 MinIO/Redis 跨存储故障矩阵仍缺少完整真实证据。
- **解决方式：** 继续使用独立容器、端口和持久卷执行故障矩阵；每项测试绑定源码 revision、Go 工具链、资源清理结果和对象内容核验，默认上传策略保持 `relay`，未取得完整证据前不切换 `presigned`。
- **验证：** Remote GPU `83f9df25`、Go `1.27.0` 下基础 Multipart smoke 与 restart smoke 均退出码 `0`；GPU 进程和业务容器未被停止或修改。
