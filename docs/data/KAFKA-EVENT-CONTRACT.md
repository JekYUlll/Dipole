...
Topic 清单位于 `internal/services/core/bootstrap/embedded/kafka_topics.go`。Message、Group、Conversation、Contact 与 Session 事件共享本规则；`message.*.created` 由 Transactional Outbox 发布，其 value、mutation metadata 和 headers 在同一消息事务中固化。
