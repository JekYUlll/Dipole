...
| OAuth callback durable handoff（Runtime 侧 provider + token lifecycle） | 无独立开关：`services/agent-runtime/src/index.ts` 完全不装配，`OAuthCallbackRuntimeConfig` 默认 `enabled=false` 且 `assertOAuthCallbackRuntimeUnavailable` 会拒绝启用 | 缺接线（组合闭环已实现） | 需真实 provider adapter + Core-owned token lifecycle SQLC 表 + Runtime 密钥轮换/retention + Gateway callback route。回滚方式：保持可选组合未装配 |
