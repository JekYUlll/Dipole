...
本轮以 `b8b27a76` 源码和 Go `1.27.0` 执行通过，覆盖多分片乱序、同编号替换、完成、对象内容校验和重复 Abort；容器退出后已自动清理。该基础证据不覆盖客户端断网重试、服务重启恢复、预签名默认切流或完整 MinIO/Redis 故障矩阵。

随后以 `10cccdd3` 源码和同一用户态 Go 工具链完成 `multipart-restart-smoke`：首个分片写入后重启带持久卷的 MinIO，续传并 Complete 后对象内容匹配，容器和数据卷均已清理。该结果覆盖存储服务重启恢复，不代表客户端断网重试或预签名默认切流已完成。
