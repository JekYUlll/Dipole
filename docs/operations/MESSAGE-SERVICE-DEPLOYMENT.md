...
启动验收要求账号可读写 Message、Conversation Sequence allocator、Sync、Outbox 所需表，可读取 migration ledger，同时无法读取任何 Core 表。

## 渐进切换

1. 执行 `go run ./cmd/tools/migrate -direction up`，确认全部节点 schema 版本一致；`000002` 完成后再滚动发布会分配 `message_seq` 的 Message 节点。
...
隔离候选拓扑可以使用 `SMOKE_INBOX_PROJECTOR=1 SMOKE_MESSAGE_FLOW=1 scripts/smoke-microservice-isolated-images.sh` 加载 `deploy/microservices/inbox-projector.yml`，验证 Gateway 发送后由 Sync projector 异步物化目标用户 Inbox。该参数只作用于独立 smoke project，退出时自动清理；默认 smoke 仍使用 atomic 路径。
