...
6. 开发验证优先直接部署当前工作分支到 Remote GPU 的隔离 Compose project；开发容量已获授权，无需为既有 GPU 任务重复等待或预防性停机。仍使用独立目录、端口、凭据和项目名，不触碰其他用户进程。缺失运行依赖时可经 `sudo` 安装并在运行记录中注明；提交绑定候选只用于需要留档的演练、性能结论和合并前验收，不阻塞日常开发验证。
