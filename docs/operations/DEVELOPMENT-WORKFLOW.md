...
- 当前 Agent 主线保持一个连续 worktree，直到完成可体验的端到端闭环；前端仅在独立长期分支推进。除并发会修改同一运行链路外，不为测试、文档或小修复额外创建 feature 分支。
- 一次只推进一个可验收切片。代码、测试和必要文档属于同一切片；以用户可体验闭环、存储迁移边界或明确回滚点作为提交边界，避免按单个测试、配置或文档变更拆分提交。
...
5. 在一个完整闭环或明确回滚边界形成后推送并合并；同一主线中的中间验证保留在 worktree 和部署记录中，不单独创建提交。
6. 开发验证优先直接部署到 Remote GPU 的隔离 Compose project；允许 GPU 任务并行，仍使用独立目录、端口、凭据和项目名，不触碰其他用户进程。提交绑定候选用于需要留档的演练、性能结论和合并前验收。
