...
| OpenTelemetry、Eval、任务成功率 | 部分完成 | OTel 设计、模型调用审计、五类 Eval harness 与 `reviewed_shadow` 窗口汇总契约 | 对受控、人工评审的终态 Shadow 报告输出样本量、成功率、五类通过率与失败分类；仍需归档固定任务集、trace 链接和共享环境观察窗口。完成这些证据后才能填写任务成功率 `[XX]%`。 | P0 |
