...
| OpenTelemetry、Eval、任务成功率 | 部分完成 | OTel 设计、模型调用审计、五类 Eval harness、`reviewed_shadow` 窗口汇总与持久化 Run Trace 绑定 | 对受控、人工评审的终态 Shadow 报告输出样本量、成功率、五类通过率、失败分类和 Run Trace ID；仍需归档固定任务集与共享环境观察窗口。完成这些证据后才能填写任务成功率 `[XX]%`。 | P0 |
