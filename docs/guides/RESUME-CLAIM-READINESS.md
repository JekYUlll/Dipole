...
| OpenTelemetry、Eval、任务成功率 | 缺口 | OTel 设计、模型调用审计、合成 Project Guardian 预筛语料 | 增加版本化 Eval harness：结果成功、tool trajectory、permission、成本四类断言；在固定任务集上输出样本量、成功率、失败分类与 trace 链接。完成后才能填写任务成功率 `[XX]%`。 | P0 |
