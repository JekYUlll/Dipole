...
- **限制：** 当前路径使用 owner 控制 RPC；TS receipt v2、Core commit service、Agent-only internal RPC、低敏回包校验和独立 active executor 契约已固定，但 Temporal Activity、Core bootstrap、授权演练和共享环境证据尚未接入，不能宣称 active Agent 已自动写入长期 Memory。
