...
- **限制：** Remote GPU 的隔离候选已使用同一代码 revision `71bf6507` 构建 Gateway、Core、Message、Sync 与 Agent，并验证认证、Task admission、Temporal 终态和 Timeline。该验收只覆盖 read-shadow 体验，不能外推为任务成功率、active authority 或写 Capability；基础 Compose 默认仍关闭控制面。Pencil canonical 创建画板和视觉回归继续待补。
