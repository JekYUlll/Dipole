...
Gateway、Core、Message、Sync 和 Search 有独立进程与接口边界，可以分别部署。
当前默认体验使用单节点基础设施，Kafka 复制因子为 1；它用于证明消息与 Agent 产品
闭环，不覆盖基础设施节点永久丢失。Worker 重启恢复证明 Temporal 执行恢复与应用
幂等，不能推导 MySQL、Kafka 或 Redis 集群故障下的可用性。集群实验需另外说明
拓扑、故障方式、数据持久性和观测结果，简历不承诺无限定的零丢失。
