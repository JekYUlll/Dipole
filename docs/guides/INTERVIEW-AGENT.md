...
### 哪个真实任务最能说明 Temporal 的价值？

协作总结任务：先读取授权会话并检索依据，缺少关键信息时询问任务所有者；等待回复或
截止时间后生成草稿，由用户修改并批准后发布。等待期间允许 Worker 重启，仍继续同一个
Task。相比一次聊天请求，这里有跨进程生命周期的人机交互、持久定时器和外部写入。

模型负责提炼、识别缺口和生成报告；Workflow 负责确定性的状态转换与等待。模型调用在
Activity 中，各阶段结果单独持久化；Core 复核审批绑定，并通过幂等消息命令发布。草稿
保存为受用户鉴权的两版 Artifact。一次任务最多询问一轮，正文可在审批前替换，审批后不
重新生成正文。输入与检索材料始终按不可信数据处理。

演示入口为聊天顶部“协作总结”。真实验证覆盖 owner 补充后重启恢复、编辑后批准仅发一条，
以及无人补充时到期生成、拒绝零发送。当前不宣称多参与者任务收集、循环提醒或无限多轮规划。
