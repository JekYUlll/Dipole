...
- Search 服务的入口装配边界位于 `internal/services/search/bootstrap/`；Search runtime 和测试已在该目录直接组合 Search application、Elasticsearch 与平台 runtime，Internal RPC 暂由窄 compatibility adapter 承接，旧共享 runtime 路径由结构门禁阻止回流。
- Message 服务的入口装配边界位于 `internal/services/message/bootstrap/`；runtime 与配置校验测试已在该目录直接组合 Message infrastructure 和平台能力，少量 Kafka handler、Outbox、Lazy Core 与 Internal RPC helper 暂由兼容入口承接，旧共享 runtime 路径由结构门禁阻止回流。
- Sync 服务的入口装配边界位于 `internal/services/sync/bootstrap/`；runtime、数据库权限校验和测试已在该目录直接组合 Sync infrastructure 与平台能力，Internal RPC 暂由窄 compatibility adapter 承接，旧共享 runtime 路径由结构门禁阻止回流。
- Gateway 服务的入口装配边界位于 `internal/services/gateway/bootstrap/`；runtime 已在该目录直接组合 Gateway 边缘适配、Redis、Kafka、实时投递 authority 和平台 runtime，RPC、TLS 与 Kafka handler 暂由兼容入口承接，旧共享 runtime 路径由结构门禁阻止回流。
