...
- Gateway 服务的入口装配边界位于 `internal/services/gateway/bootstrap/`；runtime 与 RPC bootstrap 已在该目录直接组合 Gateway 边缘适配、Redis、Kafka、实时投递 authority 和 `internal/platform/rpc`，Kafka 热群通知聚合已归属 `internal/services/gateway/infrastructure/kafka/`，完整消息投递 handler 仍按依赖闭包渐进迁移，旧共享 runtime 路径由结构门禁阻止回流。
