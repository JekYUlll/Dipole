...
- Gateway 服务的入口装配边界位于 `internal/services/gateway/bootstrap/`；runtime 与 RPC bootstrap 已在该目录直接组合 Gateway 边缘适配、Redis、Kafka、实时投递 authority 和 `internal/platform/rpc`，Gateway Kafka handler、authority factory 与注册器均归属 `internal/services/gateway/infrastructure/kafka/`，`internal/bootstrap` 仅保留兼容转发和跨领域 projection，旧共享 runtime 路径由结构门禁阻止回流。
