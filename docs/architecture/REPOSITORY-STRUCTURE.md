...
- 已完成物理收敛的服务实现应放在 `internal/services/<service>/`；当前 Search application 位于 `internal/services/search/application/`，Sync application 位于 `internal/services/sync/application/`，通用 `internal/app` 不再承载这两个服务的 application 实现。
