...
- `internal/bootstrap/` 只负责长期运行服务的启动与生命周期管理；embedded 聚合专属的 Kafka 注册、transport 和 shadow 组合位于 `internal/bootstrap/embedded/`；回填、对账、归档和受控切换等一次性操作统一放在 `internal/operations/<domain>/`，由 `cmd/tools/` 调用。
