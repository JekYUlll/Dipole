...
- Gateway 服务的入口装配边界位于 `internal/services/gateway/bootstrap/`；runtime 与 RPC bootstrap 已在该目录直接组合 Gateway 边缘适配、Redis、Kafka、实时投递 authority 和 `internal/platform/rpc`，Gateway Kafka 热群通知、authority fence、direct-read、contact-delete、session-kick、群事件、direct-message 与 group-message handler 均归属 `internal/services/gateway/infrastructure/kafka/`，共享 Kafka handler 实现已完成迁移，旧共享 runtime 路径由结构门禁阻止回流。
