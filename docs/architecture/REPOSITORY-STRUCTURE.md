...
- `internal/bootstrap/embedded/` 负责 embedded 聚合的 composition；其 runtime 与生命周期位于 `internal/bootstrap/embedded/runtime/`，独立服务入口与 runtime 位于各自的 `internal/services/<service>/bootstrap/`。`internal/bootstrap/` 根目录仅保留迁移期 contract fixture，长期服务与一次性操作不得回流；回填、对账、归档和受控切换等操作统一放在 `internal/operations/<domain>/`，由 `cmd/tools/` 调用。
