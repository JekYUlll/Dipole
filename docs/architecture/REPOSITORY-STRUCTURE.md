...
- 已完成物理收敛的服务实现应放在 `internal/services/<service>/`；当前 Search、Message、Sync application 分别位于 `internal/services/search/application/`、`internal/services/message/application/`、`internal/services/sync/application/`，通用 `internal/app` 不再承载这些服务的 application 实现。
