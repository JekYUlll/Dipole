...
- 已完成物理收敛的服务实现应放在 `internal/services/<service>/`；当前 Core capability/Auth domain/Admin domain/User/Contact/Group domain/File domain/Session/Conversation application、Search、Message、Sync application 分别位于 `internal/services/core/`、`internal/services/search/application/`、`internal/services/message/application/`、`internal/services/sync/application/`，Core domain 按领域独立子包组织，通用 `internal/app` 只保留兼容装配入口。
