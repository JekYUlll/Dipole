...
- 2026-09-02：Subscription Auto-Reply 已接线运行时回复执行步骤(仍默认关)。订阅触发的 active read 完成后,若开关把 `subscriptionMessage` 执行器接入 Temporal read activity,则把模型摘要经审计化审批绑定 Tool Invocation(`message.system.send`、`system_message`、独立命名空间)投递到 **owner 的 direct Agent 会话**;仅在 `active`、事件带 `subscriptionId` 且摘要非空时触发。刻意选择投递到 owner Agent 会话而非触发的人-人会话,避免引入向他人会话写入的更大授权面(该更大能力如需另立债务)。**剩余债务(AD-034)**:(1) Core 侧订阅 scoped 自主审批/预授权——回复复用审批闸并按 `(task,run,capability,scope,argsSha)` 解析 grant,但自主回复无 owner 手动 Signal,需在**回复内容定案后**(而非准入时,见上条设计更正)由 Core 经订阅 scoped 自主审批 RPC 直接创建 approved grant,否则运行期 `resolveApprovalGrant` 无绑定而无法真正发送;(2) 专用 compose overlay(默认关)与静态门禁;(3) Remote GPU 隔离 smoke 断言 `agent_sent_messages=1`、审批消费一次、重放幂等。上述完成并有 reviewed 观察窗口证据前,禁止在共享环境启用。TypeScript 侧 typecheck/build/定向单测已通过。
