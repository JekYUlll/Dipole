...
- 2026-09-04：Remote GPU 的最新主线 active Agent smoke 在构建 `dipole-agent` 镜像时停留于 `npm prune`，未启动候选服务，已定向终止。交互 smoke 现对每个 build command 施加 120 至 3600 秒的超时（默认 900 秒），超时会走既有项目级 cleanup；公共 `dipole-experience` 的 11 个容器保持运行。需在新的有界窗口内重新获得 active admission 证据，当前不能复用旧 revision 或 shadow experience 的结果。
- 2026-09-04：Interactive Agent smoke 的构建入口已收窄为 `migrate/core/gateway/message/sync/agent`，避免无关 Search、Indexer 与 Timeline Repair 镜像消耗验收窗口。Agent image 将 production dependencies 分离为独立 `npm ci --omit=dev` stage，移除 observed stall 的 `npm prune`。该优化尚待 Remote GPU 重跑确认，不能据此推导 active admission 已复验。
