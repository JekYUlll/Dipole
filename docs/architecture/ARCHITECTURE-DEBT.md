...
- 2026-09-02：Shadow Eval 窗口已将最低评审样本数变为显式、回执可见的采集门禁。调用者可通过 `DIPOLE_AGENT_SHADOW_EVAL_MIN_MANIFESTS` 固定本窗口阈值；低于阈值会在读取或拷贝任何 manifest 前失败，`manifest-set.json` 同时保存所需与实际数量。默认 `1` 仅服务单样本调试；成功率 claim 仍须使用人工复核的固定多样本窗口，且不能由该门禁单独证明。
