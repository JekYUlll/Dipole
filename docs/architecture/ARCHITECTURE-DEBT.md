...
- **风险：** v24 projection 保持 shadow 观察属性，尚未接管原 `agent_tasks.status`；当前 `read_shadow` 只允许 `conversation.list`，也没有 Memory 或真实任务终态 outcome Eval。v25 的 `approved` 只保存审计结论；execution plan v1 仅允许带 CAS/回滚证据的 dry-run，prepared 服务尚未执行 projection 修改或 rollback。操作员授权当前需要受控 SQL 配置，grant 版本字段已经可核验，但实际执行器尚未读取并校验执行权限、版本和有效期。Temporal Worker 停止时 Query 会归类为 unavailable。eligible 决策不能自动切换 active。
