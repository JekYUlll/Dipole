...
- 2026-09-09：以当前主线镜像重跑隔离 `subscription_active` control smoke：Gateway proposal、第二位 reviewer 和 owner-scoped grant 通过后，Kafka 事件收敛为一个 completed durable read Task，模型调用存在且 Agent 消息为零。隔离项目已自动清理，公共 12 服务保持运行。该回归覆盖真实 control API，公共体验仍只开放短期 fixture grant 的只读验证。
- 2026-09-09：新增默认关闭的 `agent-promotion-experience.yml`，将公共 operator promotion 路由限定为仅 Gateway、显式 tenant 的维护窗口 overlay。Remote GPU 演练确认缺少绝对 `DIPOLE_INTERNAL_CERT_DIR` 会让 Docker 将缺失挂载源建为目录并导致 Gateway mTLS 启动失败；已立即按默认 experience 配置和绝对证书目录恢复，公共 12 服务运行。运行手册现要求先做 Compose 渲染、使用容器 healthcheck，并在窗口结束后不加载该 overlay 重建 Gateway。
- 2026-09-09：公共 Core 的 `experience-0b1c3f52` 早于 promotion-control RPC 装配，短窗口 Gateway route 曾返回 `503`。现仅热更 Core 至已在隔离 control smoke 使用的 `dipole-core:latest`（revision `2c81464e`）；B1 无预配置用户私聊回归通过。再次短时加载 promotion overlay 后，普通认证用户访问已装配 route 获得 `403` operator denial，trap 随后重建默认 Gateway 并复核 route disabled、公共 12 服务运行。真实 proposer/reviewer grant、evidence publication 与 owner Subscription 的共享窗口仍未开启。
- 2026-09-09：operator grant 运维工具已支持显式 `--env-file`，不再要求 source Compose 环境文件。Remote GPU 对两名临时 operator 完成 proposer/reviewer 的短期 grant/revoke 演练：授权阶段活动数为 `2`，撤销后为 `0`，同一工单保留 `4` 条审计记录。该项仅验证 operator 权限生命周期；public promotion route 继续关闭，尚未在共享环境签发 Runtime promotion grant 或执行 Subscription Active Read proposal/review。
- 2026-09-09：公共 `dipole-agent` 以独立 completed shadow Task/Run 经 mTLS Artifact RPC 发布合成、eligible promotion evidence；首次与重放均返回 artifact `98ee00db...d1ba`，说明 Artifact content-addressing 与 publication 幂等在共享 Runtime 可用。该 Task 仅用作 evidence publication probe；Gateway promotion route 保持关闭，没有 operator grant、Runtime promotion grant 或 Subscription 任务残留。
- 2026-09-09：共享 promotion 窗口已收敛为 `run-agent-promotion-window.sh open|status|close`。它只读取严格、无凭据的 JSON 部署配置，要求绝对证书目录和完整基础 Compose 文件；`open` 才叠加 Gateway-only overlay，`close` 用同一基础输入重建单个 Gateway 并验证 `promotion_route=false`。本地三项 fail-closed 回归已通过。该工具尚未在共享环境创建 proposer/reviewer、签发 Runtime grant 或启动 subscription worker。
...
- 2026-09-09：公共 `agent-subscription` worker 已短暂停止以演练回退；停止期间 B1 私聊和 B2 群 `@` 的新事件均以 `completed:completed`、单条助手回复和单次 consumed approval 收敛。worker 随后恢复 healthy，公共 12 服务健康。该证据覆盖 worker 停止对 interactive 链路的隔离，未触发或验证订阅自动回复。
- **剩余边界：** 共享体验已具备受控只读 Subscription Active Read 与 worker 回退验收；仍需独立的长期观察、真实评审 evidence 与自动回复风险门槛，才能扩大到默认订阅运行或公共自动动作。
...
- 2026-09-09：真实 DeepSeek V4 Flash Provider 的隔离 B1 smoke 已将同一 approved synthetic canary 输出为 recall/revoke 两案例 Eval suite，并经 Runtime CLI 得到 `2/2`、`10000` bps。首例为一条 completed Task、一条 model call、一条 pre-model lineage 与 canary 命中；owner revoke 后的新 Task 仍完成一次 model call，目标 Memory lineage 为零。suite/report 只保存 SHA-256、计数和布尔值，见 [`agent-memory-b1-synthetic-eval-2026-09-09`](../../benchmarks/agent-memory-b1-synthetic-eval-2026-09-09/)。revoke 回复中的 canary 仅作观察，短期会话历史可能保留旧答案；Core lineage 是撤销读取边界的权威依据。单一 synthetic 样本不构成真实语义成功率或默认持久 Memory 启用结论。
...
- **本轮进展：** `drill-agent-oauth-token-lifecycle-mysql-mtls-restart.sh` 已在 Remote GPU `de3f8974` 通过。该隔离 drill 仅起 disposable MySQL，以真实 TLS 1.3 mTLS Core listener 验证首次 active 写入、listener restart 后的精确重放和改写 metadata 的拒绝，并断言 SQLC persistence 最终仅保留一行；详情见 [`agent-oauth-token-lifecycle-restart-2026-09-04`](../../benchmarks/agent-oauth-token-lifecycle-restart-2026-09-04/)。此项不涵盖 Runtime Provider、Gateway callback route 或长期 authority。
- **本轮进展：** Core-owned SQLC expiry maintenance primitive 已进入默认未调度的 repository：单次最多清理 1000 条已到期的 `active`/`refreshed` record，原子置为 `expired` 并清除 envelope、digest、expiry 与 scope，调用方只获得行数。该 primitive 不读取或返回 token material，后续仍需受评审的独立 worker identity、调度与回滚演练。
- **本轮进展：** versioned maintenance contract 已固定 callback lease 与长期 authority 的分离：maintenance lease、Runtime key binding、digest + lease generation CAS、最小审计字段、terminal material 清理和 profile enablement gate 均已明确。后续实现必须遵循 [`MAINTENANCE.md`](../../contracts/agent-oauth-token-lifecycle/v1/MAINTENANCE.md)，不得以 callback lease 或模型可控字段替代长期 authority。
...
- 2026-09-04：默认 Gateway Compose 现装配 owner-scoped 的 `read_only` Agent Definition API，消除了 `remote + read_active` Task Control 已启动但用户无法自助创建 admission Definition 的体验断点。该 API 固定当前 principal、`conversation.list/read` 与 wildcard conversation scope；同 candidate 的 promotion grant 继续由受控审核流程签发，写 Capability、MCP、Memory、订阅与共享环境 authority 均未开启。回滚：设置 `DIPOLE_GATEWAY_AGENT_DEFINITION_ENABLED=false` 后重启 Gateway。
- 2026-09-04：Agent Task 控制面将 Core 的 admission `PERMISSION_DENIED` 映射为稳定的 HTTP `403` 与 `reason=admission_denied`。响应只列出 owner Definition 与同 candidate promotion grant 两项前置条件，未暴露 grant 存在性、候选策略、审核证据或跨 owner 数据。完整逐项 admission diagnostics 仍需 Runtime binding 与 Core 查询复用的独立契约，不能从本通用错误语义推导其已完成。
- 2026-09-04：Remote GPU 早先的主线 active Agent smoke 在 `npm prune` 无进展时被定向终止，候选未启动且公共 `dipole-experience` 始终为 11 个容器。随后 smoke 将每个 build command 限制在 120 至 3600 秒的有界窗口，构建白名单收窄为 `migrate/core/gateway/message/sync/agent`，并将 Agent production dependencies 改为独立 `npm ci --omit=dev` stage。
- 2026-09-04：上述优化已由干净 `a5482c6f` checkout 的 `agent-active-smoke-20260904170858-2756087` 隔离 Compose project 复验。`remote + read_active` 的受控 owner Definition 与短期 promotion grant 成功完成认证 Task 创建幂等、跨 owner 拒绝、`conversation.list -> conversation.read` 两步轨迹和零 Agent 消息写入断言；脚本以 `0` 退出，候选容器/卷已清理，公共项目仍为 11 个运行容器。该回执仅收口当前开发期 active admission 与构建路径，默认写入、MCP、Memory、共享租户、浏览器 HITL、Provider 质量和性能证据继续开放。
...
- 2026-09-04：Agent OAuth callback durable handoff 的 Runtime 侧 provider exchange 与 token lifecycle 组合闭环已实现，默认仍未启用。`services/agent-runtime/src/mcp/` 下新增 `OAuthCallbackProvider` 接口、`DeterministicFakeOAuthCallbackProvider` 测试双重、进程内 `TokenLifecycleStore`（`pending_exchange → active → refreshed → { revoked | expired }`）以及一个把 provider 与 lifecycle wire 到 executor 的 `OAuthCallbackHandoffProviderProcessor`。executor 语义未变：pre-effect 失败释放 lease，outcome unknown 保留 lease，声明 retryable 释放；本切片只在其**外**追加 processor + provider + lifecycle。真实 provider adapter、Core-owned token lifecycle SQLC 表、Runtime 侧密钥轮换 / retention job、Gateway callback route 注册与 `services/agent-runtime/src/index.ts` 装配仍是 release 前置。`agent_oauth_authorization_transaction_consume_enabled` 保持关闭，`OAuthCallbackRuntimeConfig` 默认 `enabled=false` 并显式拒绝 `assertOAuthCallbackRuntimeUnavailable`。回滚：保持可选组合未装配；端到端 6 场景（duplicate notify、worker restart、lease expire、exact replay、rollback、permission denied）在 `oauth-callback-handoff-durable-runtime.test.ts` 内以纯离线组合方式覆盖，无 HTTP、无真实 token endpoint、无生产状态。
...
- 2026-09-03：Subscription Auto-Reply 的 owner Definition 已从 Compose smoke 专用 SQL 升级为公开、认证的 profile 契约。Gateway 仅转发固定 `subscription_autoreply` profile，Core 从可信 owner/tenant/Agent identity 构造确定性 Definition，并固定 `conversation.read + message.write` 与 wildcard conversation `read + write` scope；空 profile 和 `read_only` 保持既有只读行为，未知 profile fail closed。Remote GPU 隔离 Compose 已通过 API 创建 Definition/Subscription，并验证一条 owner Kafka 事件精确收敛为一个 Durable Task、一次模型调用、一次 Core 签发并消费的 `message.system.send` grant、一条 owner-Agent 回复和两条 Sync Inbox 投影；项目容器与卷已清理。共享环境启用仍需 AD-034 的 reviewed 观察窗口。
...
- 2026-09-02：Subscription Auto-Reply 的 Remote GPU 端到端 smoke 已跑通，AD-034 剩余债务收窄。
  ...
  - **AD-034 剩余**：只余一项——把订阅回复做成**独立带 checkpoint 的 durable Temporal step**，让 activity 重试对「铸→resolve→consume→send」真正幂等（当前靠消息命令 `invocationId` 防重发，但重试后 resolve 会因 grant 已消费而失败）。前几条列的 Core 自动审批、compose overlay、Remote smoke 均已落地。启用前仍需 reviewed 观察窗口证据。

- 2026-09-02（设计更正）：**自主回复的 grant 不能在订阅 Task 准入时创建**。审批 grant 的解析键是 `(task_uuid, capability_id, scope_sha256, arguments_sha256)`，其中 `arguments_sha256 = sha256(canonicalJSON({conversationId, content}))` 依赖**回复正文**；而准入(`Admit`/`authorizeTriggerSubscriptionV1`，`internal/services/agent/application/agent_execution_policy.go`)发生在模型产出摘要之前，此刻 content 未知，无法预先算出 `arguments_sha256`，因此准入时建的 grant 永远无法被 `ResolveApprovalGrant` 命中。正确做法与交互写一致——在**回复内容定案后**创建 grant，区别仅在于自主回复没有 owner 手动 Signal：需要一条 Core 侧**订阅 scoped 自主审批** RPC，在回复内容确定时由 Core 校验(订阅+pinned Definition+principal 三者 owner 一致、capability 已投影 `message.system.send`、scope 恰为 owner direct Agent 会话)后**直接落 `status=approved`、`approved_by=principalUUID` 的 grant**(复用 `InsertAgentApproval`，跳过 pending)。哈希/nonce/approvalId 用订阅专用命名空间，必须与 TS `createSubscriptionMessageExecutor` 侧一致(scope hash `dipole.agent.scope.v1`；args hash 对齐 `AgentMessageCommandToolArgumentsSHA256V1` 金标)。此更正取代下一条中"准入时自动建 grant"的说法。

- 2026-09-02：Subscription Auto-Reply 已接线运行时回复执行步骤(仍默认关)。订阅触发的 active read 完成后,若开关把 `subscriptionMessage` 执行器接入 Temporal read activity,则把模型摘要经审计化审批绑定 Tool Invocation(`message.system.send`、`system_message`、独立命名空间)投递到 **owner 的 direct Agent 会话**;仅在 `active`、事件带 `subscriptionId` 且摘要非空时触发。刻意选择投递到 owner Agent 会话而非触发的人-人会话,避免引入向他人会话写入的更大授权面(该更大能力如需另立债务)。**剩余债务(AD-034)**:(1) ~~Core 侧订阅 scoped 自主审批 RPC~~ 已落 `AuthorizeSubscriptionMessage`(回复内容定案后 Core 校验订阅 owner/scope/capability 一致并直接铸 approved grant;TS executor 先铸 grant 再 resolve/consume),Go build/vet/test + MySQL 契约 + proto stub 再生待 **Remote GPU 复核**;(2) 内联"铸→resolve→consume→send"因 consume 一次性,需把订阅回复做成**独立带 checkpoint 的 durable Temporal step** 才能对 activity 重试真正幂等(当前依赖消息命令 `invocationId` 防重复发送,但重试后 resolve 会因 grant 已消费而失败);(3) ~~专用 compose overlay(默认关)与静态门禁~~ 已落 `deploy/microservices/agent-subscription-autoreply.yml`(叠加在 `agent-subscription-active.yml` 上仅翻 `DIPOLE_AGENT_SUBSCRIPTION_MESSAGE_WRITE_ENABLED`;`agent-subscription-active.yml` 显式固定其为 `false`),`scripts/check-compose.sh` 双向断言 + 静态契约测试 `scripts/test_agent_subscription_autoreply_compose.py`;完整 `docker compose config` 渲染门禁待 **Remote GPU/CI** 复核(本机无 compose v2);(4) Remote GPU 隔离 smoke 断言 `agent_sent_messages=1`、审批消费一次、重放幂等。上述完成并有 reviewed 观察窗口证据前,禁止在共享环境启用。TypeScript 侧 typecheck/build/定向单测已通过。

- 2026-09-09：Remote GPU 已在 `a9379f71` 隔离 Compose 上重跑 Subscription Auto-Reply。确定性 stub 依请求 schema 输出 Planner 或 reply/synthesis 形态；一个 owner-scoped Kafka 事件生成一个 completed durable Task、两次有界模型调用、单条 owner direct Agent 回复、一次 `message.system.send` grant 铸造/消费和一次完成 Tool invocation。隔离项目自动清理，公共 `dipole-experience` 未重启且保持健康。该证据覆盖当前一次成功路径，重放幂等、长时间观察与共享环境写入仍是默认启用前置条件。

- 2026-09-04：勘误 `AD-034` 的早期状态记录。Subscription Auto-Reply 已完成回复合成、Core subscription-scoped 授权、默认关闭 Compose overlay、静态门禁和 Remote GPU 隔离 smoke；活动按订阅触发后将摘要投递到 owner 的 direct Agent 会话，并通过确定性 Tool Invocation 与已完成调用探测收敛 Activity 重试。`DIPOLE_AGENT_SUBSCRIPTION_MESSAGE_WRITE_ENABLED` 继续只允许与 `subscription_active` 配合，且与交互写互斥。共享环境默认保持关闭，reviewed 观察窗口、回滚负责人和效果口径仍为启用前条件。
...
- 2026-09-04：Gateway OAuth callback correlation v1 已提供 HMAC 完整性保护的 transaction、owner、issuer、redirect URI、state/browser-session 摘要与 expiry 绑定；原语默认未装配且无 HTTP route。cookie 签发、密钥轮换、callback handler 与 Provider 交换继续由 release gate 限制。
- 2026-09-04：Gateway 增加默认未装配的 OAuth callback handoff record client。它在 RPC 前校验 handoff/transaction ID、两个 SHA-256 摘要、Runtime-only envelope 与 key ID，使用可信 Gateway RequestContext 传递 principal/correlation，并拒绝过期或不匹配回包。该 seam 未注册 HTTP route，也不携带原始 authorization code。
- 2026-09-04：Gateway 增加未挂载的 OAuth callback handler。它校验 HMAC correlation cookie、浏览器 session 摘要、state、可选 issuer 和固定 redirect URI，再将 code 封装为 Runtime-only envelope，原子记录 handoff 后仅通知 Runtime 的 opaque ID。校验失败在写入前结束；通知失败返回可重试状态，重复 callback 可复用 durable handoff。cookie 签发、route 注册、Provider 配置与 token 生命周期仍关闭。
- 2026-09-04：勘误 OAuth callback handoff 的早期门禁记录。受信 correlation、Runtime claim/complete/release client、Gateway notifier 与 Core mTLS Store 装配均已存在；未完成项收敛为 correlation cookie/rotation、HTTP callback 验证、Provider code exchange、token 生命周期，以及受控 Provider 故障演练。默认部署继续没有 OAuth callback 网络入口。
...
- **本轮进展：** default-off Worker lifecycle owner 已组合现有 Temporal Runtime 与 managed startup snapshot。disabled 零 Worker 调用；构造/启动失败回收 resource；正常关闭严格执行 Worker/connection stop 后再关 Core/Artifact resource，并在任一失败下继续清理且幂等脱敏。`external_mcp_shadow` 现经专用 process owner 接入 `index.ts`，并保持基础 Compose 默认关闭；真实 route manifest、Profile 凭据、公网 DNS/TLS/readiness、共享 Shadow 演练与回滚证据仍缺失。
...
- **本轮进展：** default-off Shadow Worker bootstrap 已组合 seal definitions、deployment loader、lazy RPC resource、managed startup 与 Temporal lifecycle，固定 disabled 零 RPC/Worker、副作用交接和单 owner 回滚。`external_mcp_shadow` 已作为独占 mode 接入生产 `index.ts`：完整 Profile、Temporal、Kafka subscription trigger 与 Capability RPC 配置通过前不会创建资源；通过后专用 process owner 接管 Worker、Temporal client 与 Kafka consumer，并在停止时先收 Kafka、再停 Client/Worker。基础 Compose 继续选择 `foundation`。真实 route manifest、Profile 凭据、公网 DNS/TLS/readiness、共享 Shadow 演练和回滚证据仍缺失。
...
- **本轮进展：** operator grant 的受控预置现提供默认 dry-run 的单用途脚本。`--apply` 要求显式 Compose project、不同 actor/target、工单、原因与短期 UTC expiry，并将授予或撤销追加到 migration v59 的审计表；该脚本不开放 Runtime promotion、Gateway 路由或生产 authorizer。共享环境的双人流程、权限再授权审阅与实际故障演练继续保留为开放门禁。
...
- **本轮进展：** 增加 `memory-corpus:manifest` operator CLI，复用 source loader 的严格路径和文件权限规则，从当前进程 UID 派生 owner，并只向新的 `0600` 输出路径写入批准 manifest。operator 不再手写 corpus/review 哈希或 owner UID；CLI 输出不包含来源路径或正文。真实 owner-approved corpus、发布签名和共享环境评测证据继续待完成，默认 Memory 保持关闭。
...
- **本轮进展：** 独立 Core Runtime 已装配既有 `PersistentAgentRuntimePromotionControlServiceV1` 到 Agent gRPC server；Gateway 已有的 proposal/review/revoke 适配器在显式 `agent_promotion_enabled` 下可使用同一 SQLC 审核域模型。Remote GPU 的临时 route 探针以认证用户得到 `403`（缺 operator grant），证明请求没有因未装配退为 `503`；Gateway 随即恢复默认镜像和关闭开关。Definition 或 Subscription 创建不产生 grant；owner-reviewed grant 流程与 subscription active 体验验收仍待完成。
- **优先级：** P1
- **状态：** 处理中
...
- **本轮进展：** `subscription_active` 已在 Remote GPU 的隔离 loopback-only Compose 中完成真实 Gateway `chat.send -> Kafka -> matcher -> Temporal` 只读收据：一个 owner-scoped Task 与一个 model run 进入 completed，Agent 消息写入保持零，临时 promotion grant 在结束前撤销，Compose 项目已按标签清理。该 fixture 使用固定关键词订阅和 loopback model stub；共享 tenant 事件、外部 Provider、语义预筛质量、容量、回滚窗口与默认路径继续关闭。
- **本轮进展：** `subscription_autoreply` 已在 Remote GPU 的隔离 loopback-only Compose 中完成一次完整写入收据：认证 owner 的单条 Kafka 事件收敛为一个 completed Task、一个 completed model run、一次 consumed `message.system.send` approval、一次 completed Tool Invocation、一条 owner-Agent 回复和两条 Sync Inbox 投影；临时 grant、项目与 volumes 均已清理。运行时在订阅匹配前忽略自身 Agent 消息，避免回复回流生成递归 Task。该 fixture 使用确定性 loopback model stub，只证明受控单事件副作用和循环抑制，不代表共享 tenant、真实 Provider、重放故障注入、容量或默认启用。
...
- **本轮进展：** 复核 Message owner 的重复发送路径已实际从 Metadata locator 接入默认关闭的 Cassandra hydration，命中时不回读 MySQL 正文；缺失、身份冲突、历史无 Seq、取消或 Cassandra 错误均安全回退。补充请求取消传播回归，确保 gRPC deadline 不会被 hydration 改写为后台读取；仍待 24 小时灰度证据，不能据此退役 MySQL 正文。
...
- **本轮验证（2026-09-09）：** 公共 `dipole-experience` 已滚动 Core 与 Agent Runtime 到同一干净 revision `86d8ffdb`，新 B1 私聊和 B2 群提及均以 `completed:completed`、单条助手回复、单条 consumed approval 收敛。滚动仅涉及 `core` 与 `agent`，Route A 保持关闭；结果证明开发期体验闭环与同版本运行，不推导共享 authority、浏览器 HITL、真实模型质量、容量或成功率结论。
- **本轮验证（2026-09-09）：** 公共体验栈的显式低风险任务入口已用新认证用户复验：Gateway 接受 `POST /api/v1/agent/tasks`，Task 固定到 `lowrisk-assistant:v1` 后完成，持久 Timeline 有 5 条事件并生成一条直属助手回复。可重复脚本为 `scripts/e2e-agent-explicit-lowrisk-task.sh`；此路径不授权订阅、扩展会话范围、外部 MCP 或非批准写操作。
...
- **后续观察边界：** 2026-09-04 在 `a0e126b7` 的隔离 Remote GPU Compose 项目复验 Web Sync observability preflight，Gateway、Prometheus、Alertmanager 与 Core/Message/Sync/Gateway 指标目标均健康，退出后项目容器与卷为零。该回执不包含真实客户端样本或 24 小时 Evidence，旧 Offline 兼容窗口和 Cassandra hydration 主读继续保持关闭。
...
- **运行时验证：** Remote GPU 的 Core/Gateway 已滚动至 `82418438` 且健康。一次性开发用户完成 4-byte 预签名 Multipart：URL host 为 Gateway 公网入口、同源代理 PUT 返回 `200`、ETag 登记和 Complete 成功，Core 指标记录 `dipole_multipart_upload_terminal_total{route="direct",outcome="completed"} 1`；MinIO `9000` 没有主机监听，体验项目仍为 16 个容器。该单样本只验证候选链路，不构成 24 小时切流 receipt。
