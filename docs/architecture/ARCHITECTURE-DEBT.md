...
- **现状：** 项目已经使用 MinIO 原生 S3 Multipart Upload；超过 Web 端 `4 MiB` 阈值后由 Core 逐片接收并调用 `PutObjectPart`，默认单文件上限为 `50 MiB`、分片大小为 `5 MiB`，会话与 part ETag/Size 保存在 Redis。当前前端使用 3 路有界并发、最多 2 次指数退避重试和可选 `X-Part-SHA256` part 校验，分片请求仍经过业务服务，暂未提供预签名直传、暂停/断点恢复、强制 checksum 和整文件校验。
...
- **验证：** 当前 MinIO Multipart 单元/服务契约和 HTTP handler 测试已覆盖初始化、分片、完成、缺片、越权及 Abort 基本语义；服务端完成阶段已校验 part 实际大小、请求实际读取长度和提供的 SHA-256，前端 Multipart helper 的并发、分片边界、重试、checksum 探测和永久失败停止调度测试通过，A7 完成前不宣称已具备生产级断点续传和高吞吐直传能力。
