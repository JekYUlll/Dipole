...
- 2026-08-30：Gateway 已新增默认关闭的 Artifact metadata seam，复用受认证 Core gRPC 重新绑定 principal，并复核正文大小和 SHA-256 后丢弃正文、对象键与 metadata JSON。当前只有按 ID 的 metadata 读取，缺少 Task-scoped discoverability、浏览器正文/下载披露策略、Pencil 页面与远程运行时门禁；这些缺口继续阻止 Artifact 被描述为可用 Web 能力。
