...
- **本轮进展：** read Activity 已从单步扩展为「发现 + owner 确认读取」两步，首次让持久 `waiting_input` 出现在真实读取路径上：暂停发生在 claim 读取 Step 之前，恢复由确定性 request ID、checkpoint 候选集合与 Core 授权三重约束。到期沿用既有 `input_expired` 定时器，无新增状态。真实 approve/deny/expire receipt、共享环境窗口、该路径的 outcome/trajectory/permission 评测和多于一对 discovery 的编排仍未完成。
