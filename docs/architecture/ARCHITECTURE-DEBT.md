...
- 2026-08-31：Remote GPU 候选消息恢复演练发现停止态 `docker compose exec` 客户端会无限阻塞 readiness/计数探针。`smoke-microservice-isolated-images.sh` 已将这些调用收敛为默认 20 秒的 `SMOKE_EXEC_TIMEOUT_SECONDS`，发生该类问题时演练失败并走隔离项目清理；干净重跑 receipt 仍待归档。
