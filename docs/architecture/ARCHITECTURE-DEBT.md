...
- **风险：** 共享镜像扩大每个服务的攻击面和发布耦合；候选镜像尚未完成 Kafka authority 深度核对和回滚切换演练，因此暂不替换默认镜像。
- **下一步：** 在独立 Compose project 中继续核对 Kafka consumer ownership 与历史读取，再执行可回滚切换演练；证据完整后再评估默认 Compose 切换。
