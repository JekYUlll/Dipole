...
- **本轮进展：** active Agent Runtime 增加独占 `read_active` Temporal Activity profile；Task/Run/Event 绑定后由 Core RPC 返回权威 ExecutionContext，终态沿用显式 `runtime_id + mode`。`ai_sdk` 模式现使用显式 OpenAI-compatible Provider adapter，Provider name 绑定 route，base URL 与密钥在启动前校验；默认 `metadata` 保持零 Provider 构造。该切片只覆盖 `conversation.list/read`，Artifact、Message write 和其他 active Capability 仍 fail closed。
