...
- 2026-08-31：F2 Settings 已新增认证 `/settings` 路由与 Chat 入口，签名更新只调用当前 owner 的 profile API，同步状态只读，设备详情跳转既有低敏 `/devices` 页面，退出复用现有会话终止器。canonical Pencil desktop/mobile/四态画板、批准导出与 Chromium 视觉基线已完成；跨浏览器视觉回归仍按 AD-044 跟踪。
