...
- **本轮进展：** 新增默认 dry-run 的 `dipole-multipart-cleanup` 工具，按 MinIO `message-files/` 未完成 upload 的发起时间筛选，执行必须显式 `--execute --confirm`，输出逐 upload 状态并保留单项失败；新增按归属会话批量签发、绑定 `uploadId + partNumber` 的短期预签名 part URL contract，现有中转路径仍为默认回滚路径；Redis session 扫描、指标、客户端直传切流和真实 MinIO 执行仍待补齐。
