...
- **优先级：** P1
- **状态：** 处理中
- **2026-08-30 进展：** `project-guardian-synthetic-corpus` 已把四类 Project Guardian 关注事件与四类干扰事件收口为版本化、双 reviewer agreement 的低敏基线。它使用固定 fixture 标识，回归测试同时验证 corpus/review hash 绑定与规则候选的 precision/recall/cost 门槛；真实 Task/Run、人工受控语料、retrieval relevance、模型成本分位与共享观察窗口仍未具备，状态保持处理中。
