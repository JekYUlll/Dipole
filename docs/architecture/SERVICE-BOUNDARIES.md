...
- 聚合 `Repositories` 已显式保存 Core、Message、Sync、Agent 四类 process composition；Agent、Message、Sync 仓储访问已统一通过对应 process 分组，其余 embedded-only Core 字段继续按服务边界切片收敛，避免重新恢复扁平跨服务依赖。
