...
- Core capability、User、Contact 与 Conversation application 已迁入 `internal/services/core/application/`；User application 通过 User/File store 和对象存储依赖装配，Contact application 通过 Contact/User store、事件、通知和系统消息依赖装配，Core capability 使用最小查询接口，Conversation application 通过明确的 repository、事件和通知依赖装配，embedded 与独立 Core runtime 共用该边界。
