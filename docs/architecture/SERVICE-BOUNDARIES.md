...
| --- | --- | --- | --- | --- | --- |
| Gateway | `cmd/services/gateway` | HTTP、WebSocket、认证上下文、限流、实时连接 | Client | 无业务表；Redis 实时连接状态 | 通过 Core HTTP 反代仍保留兼容路由 |
| Core | `cmd/services/core` | Auth、User、Group、Contact、File、Conversation | Gateway、Message、Sync、Agent Capability | 用户、群组、联系人、文件元数据、Conversation State | 远程模式保留本地 Message/Sync 组合用于启动兼容和历史 HTTP 代理 |
