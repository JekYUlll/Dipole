...
- `internal/store` 仍包含少量兼容入口；旧 `internal/service` 和共享 `internal/handler` 实现已清空，兼容入口统一收纳到 `internal/compat/service/`；embedded 聚合装配已迁入 `internal/bootstrap/embedded/`，`internal/app` 仅保留兼容别名、转发和兼容测试。生产代码对 `internal/app` 的允许引用仅限 `internal/bootstrap/runtime.go` 与 `internal/bootstrap/kafka.go`，用于 embedded/旧 Kafka 装配回滚。
