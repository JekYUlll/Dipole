...
- [ ] 完成 Context Compiler 的完整检索编排，按预算组合策略、任务、会话证据、检索、Memory 和 Tool Schema。
  ...
  - [x] 固定 Agent 检索的 Core-mediated security boundary：Runtime 不直连 Search，Core 从权威 Task/Run 恢复 principal 与 scope；结果仅作为有界 `untrusted` evidence，首个契约切片不启用生产 Elasticsearch 或默认 Runtime。
