...
- [ ] F2：Search 四态、Vue 工作区、Sync 状态矩阵、desktop/mobile 恢复稿和标题栏状态已完成；Settings 认证页面已接入资料保存、设备会话安全、同步状态和退出账户。Contact、Group、File、Device 的完整页面，以及 Settings 的 Pencil frame 与视觉回归待完成。
