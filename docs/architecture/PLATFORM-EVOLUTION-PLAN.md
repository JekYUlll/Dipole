...
- [x] F2 File Directory：Pencil canonical desktop/mobile/state matrix、批准导出和 `/files` 认证只读目录已建立。Core 通过 SQLC owner-scoped cursor 查询和版本化 gRPC 暴露低敏 projection；存储 URL、对象键、校验值和上传会话不跨 HTTP 边界，下载逐项重新授权。Remote GPU 前端门禁结果随本切片提交记录。
