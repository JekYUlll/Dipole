...
- [x] 将重复发送完整返回切换为 Metadata locator + 默认关闭的 Cassandra hydration；命中返回 Timeline 正文，缺失、冲突、历史无 Seq、取消或查询错误均回退 MySQL。该能力仍需独立 24 小时灰度门禁后才能作为正文退役依据。
...
- [x] 对 Prompt Injection、越权 Tool、敏感数据外发、重复事件和循环调用进行专项测试。
