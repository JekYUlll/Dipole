...
Agent 群消息。验收结束时，显式 cleanup helper 会撤销该 Runtime grant、临时
operator 权限并关闭 promotion window；若 cleanup 失败，必须先人工完成回收再运行下一次共享验收。

```bash
PROJECT=dipole-experience GATEWAY=http://127.0.0.1:8080 \
DIPOLE_AGENT_CANDIDATE_VERSION=experience-v1 \
DIPOLE_AGENT_SUBSCRIPTION_ACTIVE_ALLOW_REVIEWED_GRANT_HELPER=1 \
DIPOLE_AGENT_SUBSCRIPTION_ACTIVE_REVIEWED_GRANT_CONFIG=/secure/dipole-promotion-window.json \
DIPOLE_AGENT_SUBSCRIPTION_ACTIVE_REVIEWED_GRANT_STATE_FILE=/secure/dipole-reviewed-subscription-grant.json \
DIPOLE_AGENT_SUBSCRIPTION_ACTIVE_REVIEWED_GRANT_COMMAND=/absolute/path/to/manage-agent-subscription-reviewed-grant.sh \
DIPOLE_AGENT_SUBSCRIPTION_ACTIVE_REVIEWED_GRANT_CLEANUP_COMMAND=/absolute/path/to/manage-agent-subscription-reviewed-grant.sh \
