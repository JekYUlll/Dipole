...
- 2026-09-09：公共 `dipole-experience` 已以干净同版本 `86d8ffdb` 滚动 Core 与 Agent Runtime。新 B1 私聊任务 `task:047b0d3a…` 与 B2 群提及任务 `task:0ef7c65d…` 均为 `completed:completed`，每项只有一条助手回复及一条 consumed approval；Core Route A 开关仍为 `false`，Runtime B1/B2 入站开关仍为 `true`。本次只重建 `core` 和 `agent`，其余体验容器保持运行。
- 2026-09-09：随后仅滚动 Core 至 `0b1c3f52a`，Agent Runtime 继续使用已验证的 `86d8ffdb` 镜像。新 B1、B2 和无 grant owner Definition fallback 均收敛为 `completed:completed`，每项精确产生一条助手回复和一条 consumed approval。重建 Core 必须显式传入绝对 `DIPOLE_INTERNAL_CERT_DIR`；相对路径缺失时 Docker 会创建同名目录并导致 mTLS 证书挂载失败。Route A 两个开关继续为 `false`。
- 2026-09-09：公共栈复验显式交互任务：新认证用户调用 `POST /api/v1/agent/tasks` 后获得 `accepted`，任务固定到 `lowrisk-assistant:v1`、完成 durable Timeline（5 条事件）并只产生 1 条直属 Agent 回复。普通任务无需 owner Definition；Subscription 继续要求 reviewed promotion grant。低敏收据见 [`agent-explicit-lowrisk-task-2026-09-09`](../../benchmarks/agent-explicit-lowrisk-task-2026-09-09/)。
...
- 2026-09-09：隔离 B1 持久 Memory E2E 已通过：已审核的同会话 canary 经 Core scoped reader、Temporal Task 和 `reply()` 进入一次确定性模型调用，任务完成且精确写入一条回复与一条 pre-model Memory lineage。该 profile 默认关闭，验收模型为 stub；它验证授权范围、审计和端到端编排，尚不构成真实 Provider 长期召回质量或自动写入策略的结论。
- 2026-09-09：同一隔离 B1 profile 已完成 owner revoke E2E：认证 owner 经 Gateway 撤销 canary 后，第二条新私聊创建独立 Task 并从 Core reader 得到空集；确定性模型精确回复 `B1_MEMORY_MISSING`，已撤销 Memory 没有新的 lineage。候选资源自动清理，公共体验仍为 11 个健康容器。
- 2026-09-09：B1 provider 模式以显式 Compose env 文件复用 DeepSeek V4 Flash，未打印或复制凭据。隔离候选中的首条与撤销后的第二条私聊均完成真实模型调用并产生非空回复；首条有 canary 的 pre-model lineage，第二条对已撤销 canary 的 lineage 为零。该验收证明真实 Provider 连通与授权边界，语义质量与成功率仍需要独立评测。
- 2026-09-09：同一 provider 验收已归档为低敏 [`receipt`](../../benchmarks/agent-memory-provider-b1-2026-09-09/)。它仅固定 revision、模型源、哈希化 Task 和 `1/1/1/0` 模型/lineage 计数；不保存自然语言输出，因此用于链路和撤销边界复核，不能取代真实语义 Eval。
