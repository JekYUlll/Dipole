...
Smoke 使用固定 localhost 验收端口、随机 Compose project 和测试专用卷，因此应在长期 profile 启动前独立执行。它生成 `agent.otel.smoke` span，验证 Collector accepted/sent 指标并按 trace ID 从 Tempo 查询完整 trace，退出时删除测试栈。基础 Compose 对 Agent 模型变量的全局插值由脚本固定为无网络的 `compose-smoke/deterministic` 占位值，Agent 服务不会启动或读取真实 Provider 凭据。Compose 启动默认限制为 300 秒；可用 `DIPOLE_AGENT_OTEL_SMOKE_STARTUP_TIMEOUT_SECONDS` 在 30 至 1800 秒内调整，超时同样执行清理。正式环境应另外归档镜像 digest、Prometheus 原始查询、Tempo trace ID、执行时间和配置 commit。
