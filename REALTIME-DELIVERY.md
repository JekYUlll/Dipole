...
Go Gateway 现在使用 Presence `NodeID` 写入严格的 `observation.schema.json`。启动检查、独立 5 秒心跳和 dependency readiness 会刷新 15 秒 TTL，记录 expected/observed authority、epoch、phase、lease deadline、原始 lease SHA-256 与有界 reason code；Redis observation 写失败按拒绝处理。消息热路径继续使用只读 reader，避免每条消息追加 observation 写。控制面后续必须按预期实例集合聚合全部未过期记录，并与 transition receipt 的 `next_sha256` 精确匹配；当前 C++ 还没有对应 observation，双 group checkpoint receipt 也未实现，因此不能只开启 reader 或只观察 Go 节点完成切流。
