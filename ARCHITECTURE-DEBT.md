...
- **恢复记录：** cutover attempt 现以不可变 manifest 绑定源/目标 authority、初始 epoch、中断预算、三阶段节点清单与 checkpoint 清单摘要，并用哈希链事件持久化每个 receipt/bundle 摘要。重启后可归约正常续切、冻结期直接回退和目标激活后二次冻结回退；外部动作执行、租约续期、超时回退与真实中断演练仍待完成。
