...
- **本轮进展：** 默认关闭的 `TrustedMcpInvocationProducer` 使用 host-owned Workflow step/ordinal 生成稳定 Invocation ID，通过 Capability route、PolicyEngine、schema 与 egress policy 派生并验证 Profile/Server/Tool/参数；输入无法携带 authority 字段。生产仍缺 server-owned terminal finish Activity 和真实路由注册。
