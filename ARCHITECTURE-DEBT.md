...
- **阶段记录：** 2026-08-28 已建立 `dipole.delivery.v1` envelope、节点批次、逐项 ACK/error 与背压契约，并固定 Kafka source coordinates 和 Go legacy adapter；C++ shadow 已接入独立 Kafka group、hiredis direct/Sentinel reader、单连接 TTL 投影、低敏 evidence v2 与 assignment readiness。真实 Kafka+Redis 回放已量化活跃 sibling 续期时的 stale connection 差异，仍不写 Gateway，当前 Go Redis Pub/Sub 流量语义未切换；Sentinel 切主演练仍待完成。
