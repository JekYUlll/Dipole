...
- **风险：** 控制面已可安全创建确定性订阅，但 Runtime 仍未消费共享 subscription 流量。在线 Shadow 已有 24 小时窗口、抓取覆盖、counter reset 和零 error 的低敏证据合同，但尚未归档真实共享环境 evidence。确定性关键词无法覆盖语义等价表达；公开候选接口当前返回有界 Definition scope 的完整交集，尚无超大 scope 分页协议。直接启用共享环境订阅模式仍会造成难以运维的策略或相关事件漏触发。
