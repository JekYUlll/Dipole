...
- **工作区记录：** attempt 创建现将 initial transition、三阶段节点清单和 checkpoint 清单 canonicalize 后不可覆盖保存在同目录，manifest 由代码生成并在每次加载时重算；恢复流程不再依赖操作员重新提供外部清单。CLI 与 lease renewer 仍待接入。
