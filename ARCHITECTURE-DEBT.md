...
- **现状：** 官方 MCP TS SDK v2 Client/Server foundation 与默认关闭的 Gateway/Runtime 网络入口已完成，当前仅投影 `conversation.list`。migration v30 与 sqlc Store 为每次调用持久化权威 tenant/principal/Agent/Task/Run、参数/结果 SHA-256、字节数、耗时和稳定终态。统一 `AgentTelemetry` 已覆盖 Task、Run、ContextCompile、逐 provider ModelCall、native/MCP ToolCall、Approval 和 Artifact Activity；Temporal Workflow 不调用 OTel，span 属性与异常保持低敏。Gateway 还使用 Redis 固定窗口按 JWT principal 对 GET/POST 统一限流，跨 Task/Run 和 Gateway 实例共享额度；超限返回 429/`Retry-After`，Redis 缺失或配置非法时 fail closed，DELETE 保留 Session 清理能力。外部 Client foundation 已要求每个 Tool 配置参数 allowlist、请求大小上限，并在发送前拒绝深层对象和常见凭据字段。未配置 OTel SDK/exporter 时 API 保持 no-op。外部 MCP Server、write/destructive Tool 均未启用。
