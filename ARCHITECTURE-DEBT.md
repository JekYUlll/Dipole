...
- **本轮进展：** 新增 readiness evidence v1，把 local preflight 与 exact Profile online drill 约束在有界时间窗，并以 canonical SHA-256 绑定 Profile policy、opaque ref、文件路径拓扑和有效上限。低敏 bundle 只保留 hash、时间与计数，schema 可供多语言解析；自定义 Transport builder 无权出证。证据尚未签名或持久化，原地 secret 内容轮换不改变 binding；真实 Shadow 归档、freshness admission、签名/审计和回滚仍待完成。
