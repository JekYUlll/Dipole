...
- **命令记录：** `dipole-realtime-cutover` 已提供 create/status/单步 advance/rollback，并在每次调用间保留 fsynced 恢复边界。运行中 lease renewer 和真实 controller crash、Kafka rebalance、Redis outage 演练仍待完成，CLI 当前不能作为生产切流完成证明。
