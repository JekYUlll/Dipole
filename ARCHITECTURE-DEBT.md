...
- **现状：** schema 已由版本化 SQL migration 管理，运行时默认关闭 `AutoMigrate`；AICallLog、Admin、File、User、Contact、Group、Conversation、Message、Sync 和 Outbox 已具备 sqlc adapter 与 GORM 回切路径。Message/Sync/Outbox Producer 已按共享事务边界整体迁移，legacy GORM adapters 仍在兼容窗口内保留。
