...
- **阶段记录：** 2026-08-28 已建立 `dipole.delivery.v1` envelope、节点批次、逐项 ACK/error 与背压契约，并固定 Kafka source coordinates 和 Go legacy adapter；C++ 已增加确定性的 created-event 纯投影，尚未接入 Kafka、Redis 或 Gateway，当前 Go Redis Pub/Sub 流量语义未切换。
