...
- **恢复记录：** cutover attempt 现以不可变 manifest 绑定源/目标 authority、初始 epoch、中断预算、三阶段节点清单与 checkpoint 清单摘要，并用哈希链事件持久化每个 receipt/bundle 摘要。单步 orchestrator 在每次动作前重载 journal，使用确定性幂等 action ID；执行失败保留原状态，首次 freeze 超过预算后自动转入源 authority 回退。重启后可归约正常续切、冻结期直接回退和目标激活后二次冻结回退；生产 Redis/Kafka executor、租约续期与真实中断演练仍待完成。
