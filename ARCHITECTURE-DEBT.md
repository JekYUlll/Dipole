...
- **现状：** migration v29 与 sqlc Store 已保存五类不可变 scoped Memory、full/compact content、priority、有效期和 provenance；Core 根据运行中的 Task/Run 固定 principal、tenant、Agent 与 conversation read scope，并使用 Task 创建时间阻止后续新增记录进入重放，撤销/过期立即 fail closed。v38 已交付默认关闭的 owner list/revoke，v39 已交付 append-only correction 与五类离线结构 Eval。v40 提供 root-wide 内容擦除收据与固定 tombstone。v41 在 Shadow Plan 同一事务内保存 `Memory -> Task` 直接引用；精确重放可补齐缺失索引，representation 漂移会回滚事务。只读影响审计按 root 保守统计 Model Call、Plan、Step、Artifact、Tool、Message Action 与 Temporal 潜在 Task，并以 root SHA-256、有界计数和 `lineageComplete` 输出低敏报告；历史 Context 只探测引用，不读取内容，存在未索引记录时显式不完整。模型完成结果早于 Plan 提交的崩溃窗口以当前 tenant/owner 范围的 `unattributedModelTasks` 计数阻断完整声明。语言中立契约固定 `contentRead=false`、`deletionAuthority=false`、`runtimeAuthority=false`。Core 擦除、TS 审计、owner 页面和 correction 各自保持独立开关，自动写入继续关闭。
- **风险：** v41 建立 Task 级保守血缘，可以定位可能包含派生事实的持久域，但尚未证明字段级副本，也未删除 Shadow plan summary、Step input/output、Artifact body/metadata、Agent Message 或 Temporal history。历史 Context 引用没有无界回填，模型调用到 Plan 之间也缺少 root-specific attribution；任一缺口非零时不能据此声明影响集合完整。真实多次纠正、语义冲突和 retrieval ranking 标注语料仍缺失，仅按 priority 的精确 scope 检索无法衡量生产 recall、precision 和 context 成本。
- **建议方向：** 下一步基于低敏影响报告定义各持久域的 retention/erasure policy、owner 可见收据和有界历史回填，再归档真实 correction/retrieval corpus并增加离线 Observation/Reflection Worker。写入策略要求来源证据、置信度、TTL、幂等键和冲突合并；基于 retrieval Eval 比较 MySQL 精确检索、Elasticsearch hybrid/vector 与 reranker。
