...
- **本轮进展：** `createTemporalMcpMultiRouteRuntime` 已把 deployment plan 中全部 route-scoped runtime 收敛到唯一 Activity 表面，构造时拒绝空集/重复 route，begin 与 resume 分别按输入或 durable checkpoint 的 route ID 选取 runtime，随后继续执行 route-local 全量绑定校验。该层无 I/O 或启动副作用；通用 Workflow、Worker registration、`index.ts`、真实路由调度与外部网络仍关闭。
