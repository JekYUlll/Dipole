...
`AdmitRun` 支持显式 `shadow|active` mode 与 candidate version；服务端固定 Runtime ID，active admission 仍必须通过持久 promotion authorizer。`CompleteRun`/`FinishRun` 同样绑定 Runtime ID 与 mode，避免 active Run 终态误写到 shadow Run；省略这些字段的旧调用按 shadow 兼容处理。`ResolveMcpContext` 仅允许受认证 `dipole-agent` 调用，transport context 不接受 principal。请求携带 Gateway 已认证 principal 与精确 Task/Run；Core 使用持久 Invocation resolver 校验 owner、运行中的 Run、固定 Definition、grant 和 resource scope。principal 不匹配统一返回 NotFound，响应包含持久 Run 的权威 `runtime_id/mode`；调用方必须验证固定 Runtime identity，不能在客户端自行提升 shadow context。
